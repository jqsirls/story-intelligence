#!/bin/bash
# Deploy Library Agent Lambda (Proper Deployment with EmailService)
set -e
...
AGENT_DIR="packages/library-agent"
PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

echo -e "${PURPLE}╔══════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${PURPLE}║            📚 DEPLOYING LIBRARY AGENT (WITH EMAIL) 📚            ║${NC}"
echo -e "${PURPLE}╚══════════════════════════════════════════════════════════════════╝${NC}"
...
# Step 3: Build library-agent
echo -e "${BLUE}📦 Step 3: Building Library Agent...${NC}"
cd "$AGENT_DIR"
...
# Copy library-agent dist
cp -r "$AGENT_DIR/dist" "$DEPLOY_DIR/"
echo -e "${GREEN}✅ Copied Library Agent dist files${NC}"

# Create Lambda handler that uses actual LibraryAgent class
cat > "$DEPLOY_DIR/dist/lambda.js" << 'LAMBDA_EOF'
const { LibraryAgent } = require('./index');
...
async function initialize() {
  ...
  try {
    ...
    // Initialize LibraryAgent with EmailService
    ...
    logger.info('Library Agent initialized with EmailService');
  } catch (error) {
    logger.error('Failed to initialize Library Agent', { error: error.message });
    throw error;
  }
}
...
exports.handler = async (event, context) => {
  try {
    ...
    if (event.httpMethod || event.requestContext) {
      ...
      // Other actions
      if (action && libraryAgent[action]) {
        const result = await libraryAgent[action](body, body.context || { user_id: body.userId });
        return {
          statusCode: 200,
          headers: { 'Content-Type': 'application/json' },
    ...
    if (action === 'health') {
      return { success: true, agentName: 'library', emailService: !!emailService };
    }
    
    if (libraryAgent[action]) {
      const result = await libraryAgent[action](event, event.context || { user_id: event.userId });
      return { success: true, data: result };
    }
    
    return { success: false, error: 'Invalid action' };
  } catch (error) {
    logger.error('Library Agent error', { error: error.message, stack: error.stack });
    return {
...
cd "$DEPLOY_DIR"
zip -r library-agent-deployment.zip . >/dev/null 2>&1
...
aws s3 cp library-agent-deployment.zip "s3://${S3_BUCKET}/library-agent-${ENVIRONMENT}-$(date +%s).zip" 2>/dev/null || {
    # Create bucket if it doesn't exist
    aws s3 mb "s3://${S3_BUCKET}" --region "${AWS_REGION}" 2>/dev/null || true
    aws s3 cp library-agent-deployment.zip "s3://${S3_BUCKET}/library-agent-${ENVIRONMENT}-$(date +%s).zip"
}

S3_KEY=$(aws s3 ls "s3://${S3_BUCKET}/" --recursive | grep "library-agent-${ENVIRONMENT}" | sort | tail -1 | awk '{print $4}')
...
if [ "$LAMBDA_EXISTS" -eq "0" ]; then
    ...
    aws lambda create-function \
        ...
        --region "$AWS_REGION" \
        --description "Storytailor Library Agent - Story management with EmailService"
else
...
echo ""
echo -e "${PURPLE}╔══════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${PURPLE}║            🎉 LIBRARY AGENT DEPLOYED WITH EMAIL! 🎉               ║${NC}"
echo -e "${PURPLE}╚══════════════════════════════════════════════════════════════════╝${NC}"
echo -e "${CYAN}Lambda Function: ${LAMBDA_NAME}${NC}"
echo -e "${CYAN}Region: ${AWS_REGION}${NC}"
echo ""
echo -e "${GREEN}✅ Library Agent is ready with:${NC}"
echo -e "   • Full EmailService integration"
echo -e "   • Story transfer request emails"
echo -e "   • Story transfer sent emails"
echo -e "   • Story transfer accepted/rejected emails"
echo ""
