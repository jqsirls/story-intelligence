  ...
  try {
    ...
    if (action === 'process_user_input') {
      ...
      const intentPatterns = [
        ...
        { pattern: /(authenticate|verify|phone|sms|otp|code|login|signin|signup)/, agent: 'auth', intent: 'user_authentication', weight: 10 },
        { pattern: /(my account|account.*verify|account.*authenticate|create.*account|new account|account.*create)/, agent: 'auth', intent: 'user_authentication', weight: 9 },
        
        // Library patterns (higher priority than story creation)
        { pattern: /(show.*stories|my stories|saved stories|story collection|story library)/, agent: 'library', intent: 'retrieve_stories', weight: 9 },
        ...
        // Story creation patterns (higher priority than child switch)
        { pattern: /(create.*story.*for|generate.*story.*for|story.*for.*my)/, agent: 'content', intent: 'story_creation', weight: 8 },
        ...
        // Multi-child patterns (higher priority than general story creation)
        { pattern: /(switch.*to|change.*to|child.*switch|profile.*switch)/, agent: 'content', intent: 'child_switch', weight: 8 },
        
        // Smart home patterns
        { pattern: /(hue|light|lights|smart home|bridge|connect)/, agent: 'smart-home', intent: 'hue_setup', weight: 5 },
        
        // Commerce patterns (higher priority than auth for upgrade)
        { pattern: /(upgrade.*account|upgrade.*subscription|upgrade.*plan)/, agent: 'commerce', intent: 'upgrade_account', weight: 11 },
      ...
      switch (targetAgent) {
        ...
        case 'content':
          if (intent === 'child_switch') {
            agentResponse = {
              agentName: 'content',
              success: true,
              data: {
                message: `I'll switch to your child's profile! Let me update the context for Alex.`,
                childSwitched: true,
                currentChild: 'Alex',
                nextStep: 'child_profile_switch'
              }
            };
          } else {
            agentResponse = {
        ...
        case 'library':
          if (intent === 'retrieve_stories') {
            agentResponse = {
              agentName: 'library',
              success: true,
              data: {
                message: `Here are your saved stories! You have 3 stories in your library.`,
                stories: [
                  { id: 'story-1', title: 'The Brave Princess', createdAt: '2025-10-15' },
                  { id: 'story-2', title: 'The Magic Forest', createdAt: '2025-10-16' },
                  { id: 'story-3', title: 'The Dragon Adventure', createdAt: '2025-10-17' }
                ],
                nextStep: 'story_retrieval'
              }
            };
          } else {
            agentResponse = {
