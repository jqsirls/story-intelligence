...
export class AgentDelegator {
  ...
  private async makeHttpRequest(endpoint: string, request: AgentRequest): Promise<AgentResponse> {
    try {
      // Map intent type to action for simplified agents
      const actionMap: Record<string, string> = {
        'create_story': 'generate_story',
        'track_emotion': 'detect_emotion',  // Emotion agent uses 'detect_emotion'
        'save_story': 'save_story',
        'get_stories': 'get_stories',
        'hue_setup': 'discover_devices',
        'synthesize_speech': 'synthesize_speech',
        'user_authentication': 'ensure_authenticated',
        'upgrade_account': 'create_checkout_session',
        'mood_update': 'detect_emotion',
        'daily_checkin': 'daily_checkin'
      };
      
      const action = actionMap[request.intent.type] || request.intent.type;
      
      // Create the request payload with both formats for compatibility
      // Flatten everything to top level for simplified agents
      const payload = {
        action,  // For simplified agents
        intent: request.intent,  // For full agents (optional)
        context: request.context,  // For full agents (optional)
        
        // Flatten all fields to top level for compatibility
        userInput: request.context.userInput,
        input: request.context.userInput,
        message: request.context.userInput,
        userId: request.userId,
        sessionId: request.sessionId,
        characterName: request.memoryState.context?.characterName || 'Frankie',
        userAge: request.memoryState.context?.userAge || request.memoryState.context?.age || 7,
        language: request.memoryState.context?.language || 'en',
        timestamp: new Date().toISOString(),
        
        // Keep memoryState for full agents
        memoryState: {
      ...
      // Check if endpoint is a Lambda ARN
      if (endpoint.startsWith('arn:aws:lambda:')) {
        return await this.invokeLambda(endpoint, payload);
      }
  ...
  private async invokeLambda(arn: string, payload: any): Promise<AgentResponse> {
    try {
      ...
      // Lambda response might be wrapped in statusCode/body format
      let agentResponse: any;
      if (lambdaResponse.statusCode && lambdaResponse.body) {
        // Parse the body if it's a string
        agentResponse = typeof lambdaResponse.body === 'string' 
          ? JSON.parse(lambdaResponse.body)
          : lambdaResponse.body;
      } else {
        agentResponse = lambdaResponse;
      }

      // Validate response structure
      if (!this.isValidAgentResponse(agentResponse)) {
        throw new Error('Invalid agent response format from Lambda');
      }
  ...
  private createMockResponse(request: AgentRequest): AgentResponse {
    ...
    let nextPhase = request.intent.conversationPhase;
    if (agentName === 'content' && (request.intent as any).type === 'STORY_CREATION') {
      nextPhase = 'character_creation' as any;
    }
