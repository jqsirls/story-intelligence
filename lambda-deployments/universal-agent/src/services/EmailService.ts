...
export class EmailService {
   ...
   * Template name mapping: 'invitation' -> 'invite', etc.
   */
  private async getTemplateId(templateName: string): Promise<string | null> {
    ...
    // Map template names to SSM parameter names
    // This maps the code's template name to the actual SSM parameter name
    const templateNameMap: Record<string, string> = {
      'invitation': 'invite',  // Code uses 'invitation', SSM has 'invite'
      'receipt': 'receipt',    // Direct mapping
      'welcome': 'welcome',    // If exists in SSM
      // Add other mappings as templates are added to SSM
    };
    const ssmTemplateName = templateNameMap[templateName] || templateName;

    // Try multiple paths in order of preference
    const pathsToTry = [
      `/storytailor-production/email/templates/${ssmTemplateName}`, // Direct path (actual SSM structure)
      `/storytailor-production/email/templates/sendgrid-${ssmTemplateName}`, // With sendgrid- prefix
      `/storytailor-production/email/sendgrid-templates/storytailor-${ssmTemplateName}`, // Alternative path
    ];

    for (const paramName of pathsToTry) {
      try {
        ...
        if (templateId) {
          ...
          this.logger.debug(`Found SendGrid template ID for ${templateName}`, { 
            templateName, 
            ssmTemplateName, 
            paramName, 
            templateId 
    ...
    this.logger.warn(`Template ID not found for ${templateName}`, { 
      templateName, 
      ssmTemplateName, 
   ...
   */
  async sendWelcomeEmail(to: string, userId: string, userName?: string, userType?: string): Promise<void> {
    ...
    if (!userType) {
      const { data: user } = await this.supabase
        ...
        .eq('id', userId)
        .single();
      userType = user?.user_type || 'other';
    }
    
    // User-type-specific template selection
    ...
    const templateName = templateMap[userType] || 'welcome';
    const templateId = await this.getTemplateId(templateName);
    
    // User-type-specific value propositions
    const valueProps: Record<string, string> = {
      'parent': 'Create magical bedtime moments with your child',
      'teacher': 'Engage your classroom with personalized storytelling',
      'therapist': 'Support therapeutic goals through narrative therapy',
      'child': 'Your personalized adventures await!'
    };
    
    const valueProp = valueProps[userType] || 'Create personalized stories';
    ...
    if (templateId) {
      ...
      const html = `<h1>Welcome to Storytailor!</h1><p>Hi ${userName || 'there'}, welcome to Storytailor!</p><p>${valueProp}</p>`;
      await this.sendEmail({
        to,
   ...
   */
  /**
   * Send parent consent email for COPPA compliance
   */
  async sendParentConsentEmail(
    to: string,
    parentName: string,
    childName: string,
    childEmail: string,
    ...
    try {
      const template = await this.templateService.renderParentConsentEmail({
        parentName,
        childName,
        childEmail,
        consentUrl,
      ...
      await this.sendEmail({
        to,
        subject: `Parental Consent Required for ${childName}'s Storytailor Account`,
        html: template,
        from: this.fromEmail
      });

      this.logger.info('Parent consent email sent', {
        to,
        childEmail,
        consentUrl
      });
    } catch (error) {
      this.logger.error('Failed to send parent consent email', {
        to,
        childEmail,
