...
export interface ParentConsentTemplateData {
  parentName: string;
  childName: string;
  childEmail: string;
  consentUrl: string;
...
export class EmailTemplateService {
  ...
  /**
   * Render parent consent email for COPPA compliance
   */
  async renderParentConsentEmail(data: ParentConsentTemplateData): Promise<string> {
    // Try to load template, fallback to inline template if not found
    let template: string;
    try {
      template = await this.loadTemplate('parent-consent-request.html');
    } catch {
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    <h1 style="color: #2c3e50;">Parental Consent Required</h1>
    <p>Dear ${data.parentName},</p>
    <p>Your child, <strong>${data.childName}</strong> (${data.childEmail}), has requested to create an account on Storytailor. 
    Because your child is under 13 years of age, we require your consent before they can use our service.</p>
    <p>Please review our <a href="${process.env.APP_URL || 'https://storytailor.com'}/privacy">Privacy Policy</a> and 
    <a href="${process.env.APP_URL || 'https://storytailor.com'}/terms">Terms of Service</a> before providing consent.</p>
    ...
    <p style="font-size: 12px; color: #7f8c8d;">
      This consent link will expire on ${data.expiresAt}. 
      If you do not provide consent, your child's account will remain inactive.
    </p>
    <p style="font-size: 12px; color: #7f8c8d;">
      If you did not request this, please ignore this email or contact us at support@storytailor.com
    </p>
  </div>
</body>
</html>`;
