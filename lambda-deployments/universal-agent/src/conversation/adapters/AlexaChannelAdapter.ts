...
export class AlexaChannelAdapter extends ChannelAdapter {
  ...
  private createCard(response: UniversalResponse, session: ConversationSession): any {
    ...
    if (session.state.currentCharacter?.appearance_url || coverUrl) {
      return {
        type: 'Standard',
        title: card.title,
        text: card.content,
        image: {
          smallImageUrl: session.state.currentCharacter?.appearance_url || coverUrl,
          largeImageUrl: session.state.currentCharacter?.appearance_url || coverUrl
        }
      };
    }
