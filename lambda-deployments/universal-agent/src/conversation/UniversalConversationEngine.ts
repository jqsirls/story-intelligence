...
export interface MessageMetadata {
  ...
  // Channel-specific metadata properties
  alexaProcessed?: boolean;
  processedIntent?: string;
  apiProcessed?: boolean;
  apiFileProcessing?: boolean;
  apiAction?: boolean | string;
  mobileVoice?: boolean;
  mobileText?: boolean;
  mobileImage?: boolean;
  ...
  formattedForWeb?: boolean;
  hasTextAlternative?: boolean;
  // Channel-specific metadata properties
  originalVoice?: boolean;
  extractedSlots?: Record<string, any>;
  clientInfo?: Record<string, any>;
  validation?: { valid: boolean; errors?: string[]; [key: string]: any };
  deviceInfo?: Record<string, any>;
  inputMethod?: string;
  captureMethod?: string;
  entities?: Array<{ type: string; value: string; [key: string]: any }>;
  webChatVoice?: boolean;
  dimensions?: string;
  fileName?: string;
  ...
  requestId?: string;
  audioQuality?: string;
  voiceAlternative?: string | boolean;
...
export interface ConversationResponse {
  ...
  channelAdaptations: Partial<Record<ConversationChannel, any>>;
...
export interface ResponseContentMetadata {
  ...
  hasTextAlternative?: boolean;
  // Channel-specific metadata properties
  originalContent?: string | AudioData | ImageData | CardData | ActionData;
  responseFormat?: string;
  compressionApplied?: boolean;
  compressionUsed?: boolean;
  supportsMarkdown?: boolean;
  batteryOptimized?: boolean;
  rateLimitInfo?: { remaining?: number; resetAt?: string; [key: string]: any };
  debugInfo?: any;
}
...
export interface ConversationState {
  ...
  channelStates: Partial<Record<ConversationChannel, any>>;
}
...
export class UniversalConversationEngine {
  ...
  // Response caching
  private responseCache: Map<string, { response: ConversationResponse; timestamp: number; ttl: number }> = new Map();
    ...
    try {
      ...
      const session: ConversationSession = {
        ...
        state: {
          ...
          channelStates: {} as Partial<Record<ConversationChannel, any>>
        },
  ...
  async processMessage(request: ConversationRequest): Promise<ConversationResponse> {
    ...
    try {
      ...
      if (cachedResponse) {
        this.logger.debug('Cache hit for conversation request', {
          requestId: request.requestId,
          cacheKey
        });
        return {
      ...
      // Cache response if cacheable (non-stateful, non-personalized responses)
  ...
  /**
   * Map channel to device type
   */
  private mapChannelToDeviceType(channel: ConversationChannel): 'voice' | 'text' | 'multimodal' | 'api' {
    const deviceTypeMap: Record<ConversationChannel, 'voice' | 'text' | 'multimodal' | 'api'> = {
      'alexa_plus': 'voice',
      'google_assistant': 'voice',
      'apple_siri': 'voice',
      'web_chat': 'text',
      'mobile_voice': 'voice',
      'mobile_app': 'multimodal',
      'api_direct': 'api',
      'agent_to_agent': 'api',
      'webhook': 'api',
      'custom': 'multimodal'
    ...
    return deviceTypeMap[channel] || 'multimodal';
  }
  ...
  private async convertToRouterRequest(
    ...
    return {
      ...
      deviceType: this.mapChannelToDeviceType(request.channel),
  ...
  private async createChannelAdaptations(
    response: UniversalResponse,
    session: ConversationSession
  ): Promise<Partial<Record<ConversationChannel, any>>> {
    const adaptations: Partial<Record<ConversationChannel, any>> = {};
  ...
  private async executeAction(action: ConversationAction): Promise<void> {
    switch (action.type) {
      ...
      case 'trigger_webhook':
        ...
        try {
          const webhookEvent = {
            type: action.webhookType || 'conversation_event',
            data: action.webhookData || {
              actionType: action.type,
              responseId: action.responseId,
              sessionId: action.sessionId,
              timestamp: new Date().toISOString()
            },
            source: 'universal-conversation-engine',
            userId: action.userId
          };
          
          // Publish webhook event - webhook delivery system will handle delivery
          await this.eventPublisher.publishEvent(
            'com.storytailor.webhook.triggered',
            webhookEvent
          );
          
          this.logger.info('Webhook triggered', {
            webhookType: action.webhookType,
            responseId: action.responseId
          });
        } catch (error) {
  ...
  private async performChannelSync(
    ...
    try {
      ...
      // Synchronization is handled by CrossChannelSynchronizer at a higher level
      // This method returns conflicts detected during sync, which are resolved by the synchronizer
      
      this.logger.debug('Channel synchronization performed', {
        syncType,
        conflictResolution,
   ...
   */
  private generateCacheKey(request: ConversationRequest): string {
    // Create cache key from request content and context (excluding session-specific data)
    const keyParts = [
      request.userId,
      request.channel,
      request.message.type,
      typeof request.message.content === 'string' ? request.message.content : JSON.stringify(request.message.content),
      request.locale
    ];
    return `conv:${keyParts.join(':')}`;
  }

  /**
   * Get cached response if available and not expired
   */
  private getCachedResponse(cacheKey: string): ConversationResponse | null {
   ...
   */
  private cacheResponse(cacheKey: string, response: ConversationResponse): void {
   ...
   */
  private isCacheable(request: ConversationRequest, response: ConversationResponse): boolean {
    // Don't cache if:
    // 1. Request contains personal/sensitive data
    // 2. Response contains stateful information
    // 3. Response is personalized
    // 4. Response contains actions or side effects
    
    if (response.actions && response.actions.length > 0) {
      return false; // Actions shouldn't be cached
    }
    
    if (response.smartHomeActions && response.smartHomeActions.length > 0) {
      return false; // Smart home actions shouldn't be cached
    }
    
    // Only cache simple text responses without personalization
    if (response.response.type === 'text' && 
        typeof response.response.content === 'string' &&
        !response.response.content.includes('{{') && // No template variables
