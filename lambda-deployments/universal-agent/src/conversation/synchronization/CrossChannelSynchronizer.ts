...
class StateManager {
  ...
  constructor(private logger: Logger) {
    ...
    try {
      ...
      this.redisClient.on('error', () => {
        // Silently handle Redis errors - fallback to Supabase
        this.redisClient = null;
      });
    } catch (error) {
      // Redis not available - will use Supabase only
      this.redisClient = null;
    }
  ...
  async getChannelState(sessionId: string, channel: string): Promise<ChannelState | null> {
    try {
      // Retrieve channel state from Supabase or Redis cache
      // First try Redis for fast access
      if (this.redisClient) {
        try {
          const cachedState = await this.redisClient.get(`channel_state:${sessionId}:${channel}`);
          if (cachedState) {
            return JSON.parse(cachedState);
          }
        } catch (redisError) {
          // Redis error - continue to Supabase fallback
          this.logger.debug('Redis get failed, using Supabase fallback', { sessionId, channel });
        }
      }
      
      // Fallback to Supabase if Redis miss or unavailable
      if (this.supabase) {
        ...
        if (this.redisClient && state) {
          try {
            await this.redisClient.setex(
              `channel_state:${sessionId}:${channel}`,
              3600, // 1 hour TTL
              JSON.stringify(state)
            );
          } catch (redisError) {
            // Redis cache failed - not critical, continue
            this.logger.debug('Redis setex failed', { sessionId, channel });
          }
        }
      ...
      this.logger.warn('Failed to get channel state', { sessionId, channel, error });
      return null;
    }
  }
    ...
    try {
      ...
      if (this.supabase) {
        ...
        if (existingState) {
          ...
          const oldState = existingState.state_data || {};
          ...
          if (error) throw error;
          updatedFields.push(...Object.keys(state));
        }
      }
      
      // Update Redis cache
      if (this.redisClient && updatedFields.length > 0) {
        try {
          await this.redisClient.setex(
            `channel_state:${sessionId}:${channel}`,
            3600, // 1 hour TTL
          ...
          // Redis cache failed - not critical
          this.logger.debug('Redis setex failed during update', { sessionId, channel });
        }
      }
      
      return updatedFields;
    } catch (error) {
      this.logger.error('Failed to update channel state', { sessionId, channel, error });
      return [];
    }
  }
  ...
  async getActiveChannels(sessionId: string): Promise<string[]> {
    try {
      ...
      if (this.redisClient) {
        try {
          const cached = await this.redisClient.get(`active_channels:${sessionId}`);
          if (cached) {
      ...
      if (this.supabase) {
        ...
        // Cache in Redis
        if (this.redisClient && channelList.length > 0) {
          try {
            await this.redisClient.setex(
              `active_channels:${sessionId}`,
              300, // 5 minute TTL
      ...
      this.logger.warn('Failed to get active channels', { sessionId, error });
      return [];
    }
  }
  ...
  async applyChanges(sessionId: string, channel: string, changes: any): Promise<void> {
    try {
      // Get current state
      const currentState = await this.getChannelState(sessionId, channel);
      
      if (!currentState) {
        throw new Error(`Channel state not found: ${sessionId}:${channel}`);
      }
      ...
      // Update state
      await this.updateChannelState(sessionId, channel, updatedState, 'change_application');
      
      this.logger.debug('Applied changes to channel state', { sessionId, channel, changesCount: Object.keys(changes).length });
    } catch (error) {
      this.logger.error('Failed to apply changes to channel state', { sessionId, channel, error });
      throw error;
    }
  }
