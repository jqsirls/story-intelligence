  ...
  async getDeliveryHistory(webhookId: string, userId: string, limit = 50): Promise<WebhookDelivery[]> {
    ...
    if (this.supabase) {
      try {
        ...
        if (deliveryError) {
          this.logger.error('Error querying webhook delivery history', { webhookId, error: deliveryError });
          return [];
        }

        // Map database records to WebhookDelivery interface
        ...
        this.logger.error('Error getting webhook delivery history', { webhookId, userId, error: error.message });
        return [];
      }
    }

    // Fallback: return empty array if Supabase not available
    this.logger.warn('Supabase not available for webhook delivery history', { webhookId });
    return [];
  }
