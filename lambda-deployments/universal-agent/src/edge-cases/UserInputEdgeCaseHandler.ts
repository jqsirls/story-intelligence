...
import { ConversationContext, UserInput, InputConflict } from '../types';
// Minimal stub to avoid missing module issues in production build
type KidCommunicationIntelligenceService = {
  preprocessAudio?: (...args: any[]) => Promise<any>;
};

// Use NetworkResilienceManager's ConflictResolution type which matches the implementation
export interface ConflictResolution {
  strategy: 'accept' | 'clarify' | 'merge' | 'prefer_recent' | 'prefer_consistent' | 'user_choice';
...
export class UserInputEdgeCaseHandler extends EventEmitter {
  ...
  async resolveContradictoryInput(
    ...
    if (contradictions.length === 0) {
      return {
        strategy: 'accept',
        conflictedFields: [],
        resolution: { accepted: currentInput },
        timestamp: new Date()
      };
    }
  ...
  async clarifyAmbiguousInput(
    ...
    const clarification = this.generateClarificationFromAmbiguity(ambiguity, context);
  ...
  private generateClarificationFromAmbiguity(
    ...
    const interpretations = ambiguity.possibleInterpretations
      .map(interp => interp.interpretation)
      .join(', ');
    
    return `I want to make sure I understand. Did you mean: ${interpretations}?`;
  }
  ...
  private async mergeContradictoryInputs(
    ...
    // Merge contradictions by taking the most recent value for each field
    const mergedInput = { ...currentInput };
    const conflictedFields: string[] = [];
    
    for (const contradiction of contradictions) {
      conflictedFields.push(contradiction.field);
      // Prefer new value (most recent)
      (mergedInput as any)[contradiction.field] = contradiction.newValue;
    }
    
    return {
      strategy: 'merge',
      conflictedFields,
      resolution: { merged: mergedInput },
      timestamp: new Date()
    };
  }
  ...
  private preferRecentInput(
    ...
    // Prefer the most recent input (currentInput)
    const conflictedFields = contradictions.map(c => c.field);
    
    return {
      strategy: 'prefer_recent',
      conflictedFields,
      resolution: { preferred: currentInput },
      timestamp: new Date()
    };
  }
  ...
  private preferConsistentInput(
    ...
    // Prefer the value that appears most consistently in conversation history
    const conflictedFields = contradictions.map(c => c.field);
    const preferredValues: Record<string, any> = {};
    
    for (const contradiction of contradictions) {
      // Check conversation history for most common value
      const historyValue = this.getMostCommonValueFromHistory(contradiction.field, context);
      preferredValues[contradiction.field] = historyValue || contradiction.previousValue;
    }
    
    return {
      strategy: 'prefer_consistent',
      conflictedFields,
      resolution: { preferred: preferredValues },
      timestamp: new Date()
    };
  }
  ...
  private requestUserChoice(
    ...
    // Request user to choose between options
    const conflictedFields = contradictions.map(c => c.field);
    const options: Record<string, any[]> = {};
    
    for (const contradiction of contradictions) {
      options[contradiction.field] = [contradiction.previousValue, contradiction.newValue];
    }
    ...
    return {
      ...
      resolution: { 
        ...
        text: 'I need you to help me choose. Which option would you like?',
        options
      },
      timestamp: new Date()
    };
  ...
  private getMostCommonValueFromHistory(field: string, context: ConversationContext): any {
    // Extract most common value from conversation history
    // Analyzes conversation history to find the most frequently occurring value for the given field
    ...
    for (const item of context.conversationHistory) {
      // ConversationHistoryItem may have different structures - check for field in various places
      const itemData = (item as any).data || (item as any).content || item;
      if (itemData && typeof itemData === 'object' && (itemData as any)[field]) {
        fieldValues.push((itemData as any)[field]);
      }
    }
  ...
  private containsAdultThemes(input: string): boolean {
    // Adult theme detection using keyword patterns and context analysis
    const adultKeywords = [
      // Violence
      'kill', 'murder', 'death', 'blood', 'weapon', 'gun', 'knife', 'fight', 'attack',
      // Inappropriate content
      'drug', 'alcohol', 'drunk', 'smoke', 'cigarette',
      // Mature themes
    ...
    // Check for phrases that suggest adult themes
    const adultPhrases = [
      'not for kids',
      'adult only',
      'mature content',
    ...
    const hasAdultPhrases = adultPhrases.some(phrase => 
      normalizedInput.includes(phrase)
    );
    
    return hasAdultKeywords || hasAdultPhrases;
  }
  ...
  // Non-standard language processing methods
  ...
  private detectSlang(input: string): { confidence: number; patterns: string[] } {
    // Common slang terms and patterns
    const slangPatterns = [
      // Internet slang
      'lol', 'omg', 'wtf', 'btw', 'imo', 'tbh', 'fyi', 'idk', 'smh', 'fr', 'ngl',
      // Texting abbreviations
      'u', 'ur', 'r', '2', '4', 'b4', 'cuz', 'w/', 'w/o',
      // Modern slang
      'yeet', 'slay', 'cap', 'no cap', 'bet', 'fire', 'lit', 'sus', 'based',
      // Emoji-like patterns
      ':D', ':)', ':(', ':/', ';)'
    ];
    ...
    // Check for repeated characters (common in slang/informal text)
    const repeatedChars = /(.)\1{2,}/g;
    if (repeatedChars.test(input)) {
      detected.push('repeated_characters');
    }
    
    // Check for excessive capitalization (common in slang)
    const excessiveCaps = (input.match(/[A-Z]/g) || []).length > input.length * 0.5 && input.length > 5;
    if (excessiveCaps) {
      detected.push('excessive_caps');
    }
    
    return {
      confidence: detected.length > 0 ? Math.min(0.9, detected.length * 0.2) : 0,
      patterns: detected
    };
  }
