...
exports.BiasDetectionEngine = void 0;
class BiasDetectionEngine {
    ...
    async detectBias(content, request) {
        ...
        try {
            ...
            const [demographicBias, genderBias, culturalBias, abilityBias, socioeconomicBias] = await Promise.all([
    ...
    async detectDemographicBias(content) {
        const biases = [];
        let score = 0;
    ...
    async detectGenderBias(content) {
        const biases = [];
        let score = 0;
    ...
    async detectCulturalBias(content) {
        const biases = [];
        let score = 0;
    ...
    async detectAbilityBias(content) {
        const biases = [];
        let score = 0;
    ...
    async detectSocioeconomicBias(content) {
        const biases = [];
        let score = 0;
    ...
    async analyzeRepresentation(content, request) {
...
- problematic_themes (array of strings)`;
    ...
    simpleRepresentationAnalysis(content) {
        ...
        const diversityScore = diversityIndicators.reduce((score, indicator) => score + (lowerContent.includes(indicator) ? 0.1 : 0), 0);
        ...
        const foundStereotypes = commonStereotypes.filter(stereotype => lowerContent.includes(stereotype));
        // Simple problematic theme detection
        const problematicThemes = [
            'violence', 'discrimination', 'exclusion', 'bullying', 'hatred'
        ];
        const foundProblematic = problematicThemes.filter(theme => lowerContent.includes(theme));
        return {
            characters: {
    ...
    calculateOverallBiasScore(biasCategories) {
...
exports.BiasDetectionEngine = BiasDetectionEngine;
//# sourceMappingURL=BiasDetectionEngine.js.map