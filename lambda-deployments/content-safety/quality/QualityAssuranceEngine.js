...
exports.QualityAssuranceEngine = void 0;
class QualityAssuranceEngine {
    ...
    async assessQuality(content, request) {
        ...
        try {
            ...
            const [narrativeCoherence, ageAppropriateness, educationalValue, emotionalResonance, creativity, parentRatingPrediction] = await Promise.all([
    ...
    async assessNarrativeCoherence(content, request) {
        ...
        if (request.contentType === 'story') {
            ...
            score = ((hasIntroduction ? 0.25 : 0) +
                (hasMainContent ? 0.5 : 0) +
                (hasConclusion ? 0.25 : 0)) * (hasCoherentFlow ? 1 : 0.7);
        }
        return Math.max(0, Math.min(1, score));
    }
    async assessAgeAppropriateness(content, request) {
        if (!request.userAge) {
    ...
    async assessEducationalValue(content, request) {
        let score = 0;
        ...
        score = ((hasLearningObjective ? 0.2 : 0) +
        ...
        if (request.contentType === 'activity' ||
            (Array.isArray(request.context?.userPreferences) && request.context.userPreferences.includes('educational'))) {
            score = Math.min(1, score + 0.1);
        }
        return Math.max(0, Math.min(1, score));
    }
    async assessEmotionalResonance(content, request) {
        let score = 0;
        ...
        score = ((hasEmotionalDepth ? 0.25 : 0) +
    ...
    async assessCreativity(content, request) {
        let score = 0;
    ...
    async predictParentRating(content, request) {
    ...
    hasStoryMiddle(content) {
        const middleIndicators = [
            'then', 'next', 'after', 'suddenly', 'meanwhile', 'however',
            'but', 'so', 'because', 'when', 'while', 'during'
        ];
        const lowerContent = content.toLowerCase();
        const indicatorCount = middleIndicators.reduce((count, indicator) => count + (lowerContent.includes(indicator) ? 1 : 0), 0);
    ...
    hasCharacterDevelopment(content) {
    ...
    hasLogicalFlow(content) {
        ...
        const lowerContent = content.toLowerCase();
        const connectorCount = connectors.reduce((count, connector) => count + (lowerContent.includes(connector) ? 1 : 0), 0);
        return connectorCount >= 2;
    }
    // Helper methods for age appropriateness
    assessVocabularyComplexity(content, age) {
    ...
    assessSentenceComplexity(content, age) {
    ...
    assessThemeAppropriateness(content, age) {
        const lowerContent = content.toLowerCase();
        let score = 1.0;
    ...
    assessLengthAppropriateness(content, age) {
    ...
    hasPositiveValues(content) {
        ...
        const lowerContent = content.toLowerCase();
        const valueCount = positiveValues.reduce((count, value) => count + (lowerContent.includes(value) ? 1 : 0), 0);
        return valueCount >= 2;
    }
    hasProblemSolving(content) {
    ...
    hasRealWorldConnection(content) {
    ...
    encouragesCriticalThinking(content) {
        const thinkingIndicators = [
            'why', 'how', 'what if', 'imagine', 'think about', 'consider',
            'wonder', 'question', 'curious', 'explore', 'investigate'
        ];
        const lowerContent = content.toLowerCase();
        const thinkingCount = thinkingIndicators.reduce((count, indicator) => count + (lowerContent.includes(indicator) ? 1 : 0), 0);
        return thinkingCount >= 2;
    }
    // Helper methods for emotional resonance
    hasEmotionalDepth(content) {
        const emotionalWords = [
            'happy', 'sad', 'excited', 'worried', 'proud', 'scared',
            'angry', 'surprised', 'disappointed', 'grateful', 'hopeful'
        ];
        const lowerContent = content.toLowerCase();
        const emotionCount = emotionalWords.reduce((count, emotion) => count + (lowerContent.includes(emotion) ? 1 : 0), 0);
        return emotionCount >= 3;
    }
    hasRelatableCharacters(content) {
    ...
    hasEmotionalResolution(content) {
    ...
    hasImaginativeElements(content) {
    ...
    generateAssessmentDetails(scores, content, request) {
        const strengths = [];
        const weaknesses = [];
        const suggestions = [];
        // Analyze each score
    ...
    getSuggestionMessage(category, request) {
        const suggestions = {
    ...
    getEmotionalComplexityMultiplier(age) {
        if (age < 5)
            return 0.7; // Simpler emotions
        if (age < 8)
            return 0.8;
        if (age < 13)
            return 0.9;
    ...
    assessContentSafety(content) {
        ...
        const lowerContent = content.toLowerCase();
        const unsafeCount = unsafeWords.reduce((count, word) => count + (lowerContent.includes(word) ? 1 : 0), 0);
        return Math.max(0, 1 - (unsafeCount * 0.2));
    }
    assessPositiveValues(content) {
        const positiveWords = [
    ...
    hasCoherentFlow(content) {
        const flowIndicators = [
            'first', 'second', 'then', 'next', 'after', 'finally',
            'however', 'therefore', 'because', 'since', 'although'
        ];
        const lowerContent = content.toLowerCase();
        const flowCount = flowIndicators.reduce((count, indicator) => count + (lowerContent.includes(indicator) ? 1 : 0), 0);
        return flowCount >= 2;
    }
    async healthCheck() {
        try {
...
exports.QualityAssuranceEngine = QualityAssuranceEngine;
//# sourceMappingURL=QualityAssuranceEngine.js.map