...
class ProfanityFilter {
    ...
    detectObfuscatedProfanity(content) {
        const found = [];
        let severity = 0;
    ...
    detectInappropriateSlang(content) {
        const found = [];
        let severity = 0;
    ...
    detectContextualInappropriateness(content, request) {
        const found = [];
        let severity = 0;
        ...
        if (request.contentType === 'activity' || (Array.isArray(request.context?.userPreferences) && request.context.userPreferences.includes('educational'))) {
    ...
    checkAgeSpecificLanguage(content, age) {
        const found = [];
        let severity = 0;
        if (age < 5) {
