...
exports.PersonalInfoFilter = void 0;
class PersonalInfoFilter {
    constructor(logger) {
        this.name = 'personal_info_filter';
        this.priority = 60;
        this.enabled = true;
        this.logger = logger;
    }
    ...
    detectPotentialNames(content) {
        ...
        return potentialNames.filter(name => !commonPhrases.some(phrase => phrase.toLowerCase() === name.toLowerCase()));
    }
    detectBirthDates(content) {
    ...
    detectSuspiciousUrls(content) {
        ...
        return urls.filter(url => suspiciousPatterns.some(pattern => pattern.test(url)));
    }
    detectFinancialInfo(content) {
    ...
    sanitizeBirthDates(content) {
        ...
        let sanitized = content;
        for (const pattern of birthDatePatterns) {
            sanitized = sanitized.replace(pattern, (match) => match.replace(/\d{1,2}\/\d{1,2}\/\d{4}|\d{1,2}-\d{1,2}-\d{4}/g, '[DATE_REMOVED]'));
        }
        return sanitized;
    }
    sanitizeSuspiciousUrls(content) {
        const urlPattern = /https?:\/\/[^\s]+/gi;
...
exports.PersonalInfoFilter = PersonalInfoFilter;
//# sourceMappingURL=PersonalInfoFilter.js.map