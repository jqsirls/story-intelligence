...
export class InappropriateContentHandler {
  ...
  private readonly educationalOpportunities: Partial<Record<InappropriateCategory, {
    topic: string;
    ...
    [InappropriateCategory.INAPPROPRIATE_RELATIONSHIPS]: {
      topic: 'Safe Relationships and Boundaries',
      ageAppropriateExplanation: 'Friendships should be with people our own age and always feel safe and comfortable.',
      teachingMoment: 'Let\'s create stories about healthy friendships and trusted adults who keep us safe.',
      parentGuidance: 'Consider discussing safe relationships, secrets that are not okay, and how to ask for help.'
    },
    [InappropriateCategory.PERSONAL_INFORMATION]: {
  ...
  private generateEducationalOpportunity(
    ...
    if (userAge && userAge < 6) {
      ...
      ageAppropriateExplanation = ageAppropriateExplanation
        .replace(/\b\w{8,}\b/g, (match: string) => {
