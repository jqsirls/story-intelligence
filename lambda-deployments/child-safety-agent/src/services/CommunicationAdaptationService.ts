  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      await this.openai.chat.completions.create({
        model: process.env.OPENAI_MODEL_CONVERSATION || process.env.OPENAI_MODEL || 'gpt-5',
...
Respond with only the adapted message, maintaining the same meaning and tone.`;
  ...
  private adaptStructure(
    ...
    for (const need of specialNeeds) {
      const rules = this.adaptationRules[need.type] as any;
      if (!rules) continue;

      if ((rules && rules.breakIntoSteps) || (rules && rules.breakIntoSmallSteps)) {
