...
describe('CrisisInterventionSystem', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(crisisSystem, 'conductCrisisAssessment').mockImplementation(async (_i: any, _s: any, userInput: string) => {
      const suicidal = /kill myself|suicide|want to die/i.test(userInput);
      const selfHarm = /hurt myself/i.test(userInput) && !suicidal;
      return {
        riskLevel: suicidal ? 'imminent' : selfHarm ? 'high' : 'low',
        suicidalIdeation: suicidal,
        selfHarmRisk: suicidal || selfHarm,
        homicidalIdeation: false,
        psychosis: false,
        substanceUse: false,
        protectiveFactors: [],
        riskFactors: suicidal ? ['Suicidal ideation'] : selfHarm ? ['Self-harm'] : [],
        immediateInterventionRequired: suicidal || selfHarm,
        mandatoryReporting: suicidal || selfHarm,
