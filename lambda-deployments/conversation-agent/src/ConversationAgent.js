...
const HueConversationIntegration_1 = require("./integrations/HueConversationIntegration");
class ConversationAgent {
    constructor(config) {
        ...
        this.hueIntegration = new HueConversationIntegration_1.HueConversationIntegration(this.logger);
    }
    async initialize() {
        try {
    ...
    async startConversation(connectionId, userId, sessionId, metadata) {
        ...
        try {
            ...
            // Initialize Hue lighting for conversation
            try {
                const hueConnected = await this.hueIntegration.initializeHueConnection(userId, metadata?.roomId);
                if (hueConnected) {
                    this.logger.info('Hue lighting initialized for conversation', { userId, conversationId });
                }
            }
            catch (hueError) {
                this.logger.warn('Failed to initialize Hue lighting', { error: hueError, userId });
            }
    ...
    async handleTextResponse(response) {
        ...
        // Update Hue lighting based on response content and detected emotions
        if (response.metadata?.emotion && response.metadata?.userId) {
            try {
                await this.hueIntegration.handleEmotionLighting(response.metadata.userId, response.metadata.emotion, response.metadata.emotionIntensity || 0.5, response.metadata.roomId);
            }
            catch (hueError) {
                this.logger.warn('Failed to update Hue lighting for emotion', { error: hueError });
            }
        }
    }
    ...
    async handleSpecialMoment(userId, momentType, roomId) {
        try {
            await this.hueIntegration.handleSpecialMoment(userId, momentType, roomId);
            this.logger.info('Special moment handled with Hue lighting', { userId, momentType });
        }
        catch (error) {
            this.logger.warn('Failed to handle special moment with Hue', { error, userId, momentType });
        }
    }
    /**
     * Update Hue lighting for story context
     */
    async updateStoryLighting(userId, storyType, storyProgress, characterName, roomId) {
        try {
            await this.hueIntegration.handleStoryLighting(userId, storyType, storyProgress, characterName, roomId);
            this.logger.info('Story lighting updated', { userId, storyType, storyProgress });
        }
        catch (error) {
            this.logger.warn('Failed to update story lighting', { error, userId, storyType });
        }
    }
    /**
     ...
     */
    isHueConnected() {
        return this.hueIntegration.isHueConnected();
    }
    ensureInitialized() {
