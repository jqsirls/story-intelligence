...
class ElevenLabsAgentClient {
    ...
    async connect(userAge, userName, isReturningUser) {
        try {
            ...
            // Build Frankie's system prompt
            const systemPrompt = prompts_1.FrankieSystemPrompt.build({
                userAge: userAge || 7,
                mode: 'voice',
                userName,
                isReturningUser
            ...
            this.ws = new ws_1.default(wsUrl, {
                headers: {
                    ...
                    'X-System-Prompt': Buffer.from(systemPrompt).toString('base64') // Send system prompt in header
                }
            });
            this.ws.on('open', () => {
