...
export class VocabularyAdapter {
  ...
  async adaptText(
    ...
    try {
      ...
      throw new AdaptationError(`Failed to adapt vocabulary: ${message}`, { text, profile: profile.id });
    }
  }
  ...
  async learnPreference(
    ...
    try {
      ...
      const message = (error && typeof error === 'object' && 'message' in error) ? String((error as any).message) : String(error);
      throw new AdaptationError(`Failed to learn vocabulary preference: ${message}`, { userId, originalWord, preferredWord });
    }
  }
  ...
  async getVocabularyStats(userId: string): Promise<{
    ...
    try {
      ...
      const message = (error && typeof error === 'object' && 'message' in error) ? String((error as any).message) : String(error);
      throw new AdaptationError(`Failed to get vocabulary stats: ${message}`, { userId });
    }
  }
  ...
  private async getVocabularyAdaptations(
    ...
    try {
      ...
      const message = (error && typeof error === 'object' && 'message' in error) ? String((error as any).message) : String(error);
      throw new AdaptationError(`Failed to get vocabulary adaptations: ${message}`, { vocabularyLevel, ageGroup });
    }
  }
