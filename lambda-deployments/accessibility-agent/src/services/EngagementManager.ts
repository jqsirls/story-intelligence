...
export class EngagementManager {
  ...
  async performEngagementCheck(
    ...
    try {
      ...
      throw new AdaptationError(`Failed to perform engagement check: ${message}`, { userId, sessionId, checkType });
    }
  }
  ...
  async updateEngagementLevel(
    ...
    try {
      ...
      const message = (error && typeof error === 'object' && 'message' in error) ? String((error as any).message) : String(error);
      throw new AdaptationError(`Failed to update engagement level: ${message}`, { userId, sessionId, checkId });
    }
  }
  ...
  async getEngagementInsights(
    ...
    try {
      ...
      const message = (error && typeof error === 'object' && 'message' in error) ? String((error as any).message) : String(error);
      throw new AdaptationError(`Failed to get engagement insights: ${message}`, { userId });
    }
  }
