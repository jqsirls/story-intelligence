  ...
  private updateRelationshipMetrics(
    ...
    if (interaction.effectiveness > 0.7) {
      ...
      updated.successfulInteractionPatterns.push(
        `state:${String(interaction.emotionalState)}|traits:${Object.keys(interaction.personalityUsed || {}).length}|eff:${String(interaction.effectiveness)}`
      );
    }
    ...
    if (interaction.effectiveness < 0.4) {
      updated.challengingScenarios.push(
        `state:${String(interaction.emotionalState)}|phase:${String(interaction?.context?.conversationPhase)}|eff:${String(interaction.effectiveness)}`
      );
    }
