...
const shared_types_1 = require("@alexa-multi-agent/shared-types");
const ElevenLabsModelConfig_1 = require("./config/ElevenLabsModelConfig");
...
const IPAuditService_1 = require("./services/IPAuditService");
const ColorExtractionService_1 = require("./services/ColorExtractionService");
const models_1 = require("./config/models");
// Scene analysis prompt directives (used to generate cinematic scene paragraphs for cover + beats).
const SECTION_SYS = `
...
Include a clear cause-and-effect moment (what just happened and what is happening now).
Include at least one "motion proof" detail: dust, splashes, wind-tugged clothing, knocked objects, footprints, bent grass, ripples, sparks, etc.`.trim();
...
class RealContentAgent {
    constructor(config) {
        ...
        this.ipAuditService = null;
        this.contextualAnalyzer = null; // ContextualSafetyAnalyzer - lazy loaded
        // Initialize logger
        ...
        const elevenLabsKey = config.elevenlabs.apiKey && config.elevenlabs.apiKey !== 'placeholder'
            ...
            : undefined;
        this.multiCharacterDialogueService = new MultiCharacterDialogueService_1.MultiCharacterDialogueService(this.logger, elevenLabsKey);
        this.characterVoiceManager = new CharacterVoiceManager_1.CharacterVoiceManager(this.logger);
        this.soundEffectsService = new SoundEffectsService_1.SoundEffectsService(this.logger, elevenLabsKey);
        this.musicCompositionService = new MusicCompositionService_1.MusicCompositionService(this.logger, elevenLabsKey);
        this.voiceDesignService = new VoiceDesignService_1.VoiceDesignService(this.logger, elevenLabsKey);
        this.voiceRemixingService = new VoiceRemixingService_1.VoiceRemixingService(this.logger, elevenLabsKey);
        this.characterVoiceGenerator = new CharacterVoiceGenerator_1.CharacterVoiceGenerator(this.logger, elevenLabsKey);
        // Initialize color extraction service (for HUE integration)
        this.colorExtractionService = new ColorExtractionService_1.ColorExtractionService(this.logger);
        // Initialize IP detection service (Redis will be set in initialize() if available)
        this.ipDetectionService = new IPDetectionService_1.IPDetectionService();
        // Initialize IP audit service if Supabase is configured
        if (this.config.supabase.url && this.config.supabase.anonKey) {
            this.ipAuditService = new IPAuditService_1.IPAuditService(this.config.supabase.url, this.config.supabase.anonKey);
        }
     ...
     */
    buildBeatsArrayFromStoryText(storyText) {
    ...
    async generateStory(request) {
        ...
        try {
            ...
            if (phase === 'story_planning' || phase === 'greeting') {
                ...
                const requestWithAssets = request;
                ...
                if (shouldGenerateAssets) {
                    try {
                        ...
                        const images = await this.generateStoryImages({ title: story.title, keyBeats: story.keyBeats }, characterName, characterTraits, characterId, 'batch');
                ...
                return {
                    ...
                    speechText: this.formatStoryResponseWithAttribution(coverImageUrl
                        ? `Here's your story: ${story.title}! ${story.summary}`
                        : `Here's your story: ${story.title}! ${story.summary} Would you like me to create the illustrations?`, ipAttributions)
                };
            }
            ...
            if (phase === 'beat_confirmed' && request.beatNumber) {
                ...
                return {
                    ...
                    assetsStatus: {
                        ...
                        beats: [
                            updatedStory.beatImages?.some((b) => b.beatNumber === 1) ? 'ready' : 'pending',
                            updatedStory.beatImages?.some((b) => b.beatNumber === 2) ? 'ready' : 'pending',
                            updatedStory.beatImages?.some((b) => b.beatNumber === 3) ? 'ready' : 'pending',
                            updatedStory.beatImages?.some((b) => b.beatNumber === 4) ? 'ready' : 'pending',
                        ],
                        audio: updatedStory.audioUrl ? 'ready' : 'pending',
                    },
            ...
            if (phase === 'finalize') {
                ...
                const priorBeatHashes = Array.isArray(priorAssetHashes?.beatHashes) && priorAssetHashes.beatHashes.length === 4
                ...
                const nextBeatImages = beatImagesAll.filter((b) => {
                    const bn = b?.beatNumber;
                    if (typeof bn !== 'number' || bn < 1 || bn > 4)
                        return false;
                    if (!b?.imageUrl)
                        return false;
                    if (!priorBeatHashes)
                ...
                const assetsStatus = {
                    ...
                    beats: artEnabled ? ([
                        beatImages.some((b) => b?.beatNumber === 1 && b?.imageUrl) ? 'ready' : 'pending',
                        beatImages.some((b) => b?.beatNumber === 2 && b?.imageUrl) ? 'ready' : 'pending',
                        beatImages.some((b) => b?.beatNumber === 3 && b?.imageUrl) ? 'ready' : 'pending',
                        beatImages.some((b) => b?.beatNumber === 4 && b?.imageUrl) ? 'ready' : 'pending',
...
Visual moment: ${beat.description}`;
            ...
            this.logger.info(`Generating ${type} image with ${shared_types_1.DEFAULT_IMAGE_MODEL}`, {
                attempt,
            ...
            const imageGenerationPromise = this.openai.images.generate({
                model: shared_types_1.DEFAULT_IMAGE_MODEL,
                prompt,
                size: '1024x1024',
...
Visual moment: ${beat.description}`;
        this.logger.info(`Generating beat ${beatNumber} with ${shared_types_1.DEFAULT_IMAGE_MODEL}`);
    ...
    async generateFullStory(request) {
        ...
        if (request.storyId && request.storyId.startsWith('temp_') === false) {
            ...
            const { data: updatedStory, error: updateError } = await this.supabase
                ...
                scene_art_urls: images.beatImages.map((img) => img.imageUrl),
                audio_url: audioUrl,
                asset_generation_status: {
                    overall: 'ready',
                    assets: {
        ...
        return {
            ...
            speechText: this.formatStoryResponseWithAttribution(`Here's your story: ${story.title}! ${story.summary}`, ipAttributions)
        };
    }
    /**
     ...
     */
    async updateStoryBeatImage(storyId, beatNumber, imageUrl, story) {
        try {
            const beat = story.keyBeats?.[beatNumber - 1];
            ...
            if (this.tempStoryCache.has(storyId)) {
                ...
                const idx = updatedBeatImages.findIndex((b) => b?.beatNumber === beatNumber);
                if (idx >= 0)
                    updatedBeatImages[idx] = nextBeat;
                else
                    updatedBeatImages.push(nextBeat);
                updatedBeatImages.sort((a, b) => a.beatNumber - b.beatNumber);
    ...
    async generateStoryContent(request) {
        ...
        const inclusivityTraits = traits.inclusivityTraits || [];
        const inclusivityDescription = inclusivityTraits.length > 0
            ? inclusivityTraits.map((t) => t.name || t.description || t).join(', ')
            : '';
...
- Personality: ${traits.personality || 'brave and kind'}${inclusivityDescription ? `\n- Physical characteristics: ${inclusivityDescription}` : ''}${traits.ethnicity?.length ? `\n- Ethnicity: ${Array.isArray(traits.ethnicity) ? traits.ethnicity.join(' and ') : traits.ethnicity}` : ''}${traits.appearance?.clothing ? `\n- Clothing/Style: ${traits.appearance.clothing}` : ''}${traits.appearance?.hair_style ? `\n- Hair: ${traits.appearance.hair_color}, ${traits.appearance.hair_style}` : ''}${traits.appearance?.eye_color ? `\n- Eyes: ${traits.appearance.eye_color}` : ''}${traits.appearance?.skin_tone ? `\n- Skin tone: ${traits.appearance.skin_tone}` : ''}${traits.appearance?.distinguishing_features ? `\n- Notable features: ${traits.appearance.distinguishing_features}` : ''}${storyTypePrompt}${wordCountText}${vocabularyText}
...
Format your response as JSON:
{
  ...
  "keyBeats": [
     ...
     */
    getProgressMessage(imageType, attempt, maxAttempts) {
    ...
    async generateImageWithReferences({ prompt, referenceUrls, targetRating = 'G', characterName, expectedTraits = [], maxRetries = 2, size = '1024x1024', quality = 'high', storyId, imageType = 'image', useEditMode = false }) {
        // Lazy-load services
        const { ImageReferenceService } = await Promise.resolve().then(() => __importStar(require('./services/ImageReferenceService')));
        const { ImageSafetyReviewService } = await Promise.resolve().then(() => __importStar(require('./services/ImageSafetyReviewService')));
        const refService = new ImageReferenceService(this.openai, this.logger);
        const safetyService = new ImageSafetyReviewService(this.openai, this.logger, models_1.MODEL_CONFIG.VISION);
        const referenceCount = Array.isArray(referenceUrls) ? referenceUrls.length : 0;
        // Download reference images as OpenAI File objects ONLY when explicitly editing an existing image
        let referenceFiles = [];
        if (useEditMode === true && referenceUrls && referenceUrls.length > 0) {
            try {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            ...
            try {
                // Generate image (default) vs edit (explicit opt-in only)
                let response;
                const shouldUseEdit = useEditMode === true && referenceFiles.length > 0;
                if (process.env.DEBUG_IMAGE_PROMPTS === '1') {
                    this.logger.info('DEBUG_IMAGE_PROMPTS', {
                        method: shouldUseEdit ? 'images.edit' : 'images.generate',
                        referenceCount,
                        promptFirst400: currentPrompt.substring(0, 400)
                    });
                }
                if (shouldUseEdit) {
                    ...
                    if (useEditMode === true && referenceFiles.length === 0) {
                        this.logger.warn('useEditMode requested but no reference files available; using generate()');
                    }
                ...
                const ratingAcceptable = safetyService.isRatingAcceptable(review.rating, targetRating);
                ...
                if (attempt < maxRetries) {
                    ...
                    if (!review.is_child_safe) {
                        ...
                        throw new Error(`Image failed safety review after ${maxRetries + 1} attempts. ` +
     ...
     */
    // Analyze story beats into a single cinematic keyframe paragraph (≤120 words).
    async analyzeSceneForVisualDynamism(beatText, label, isCover = false, fullStory) {
        try {
            const response = await this.openai.chat.completions.create({
                model: models_1.MODEL_CONFIG.TEXT,
                messages: [
                    {
                        role: 'system',
     ...
     */
    async generateStoryPaletteJourney(fullStory) {
        const { FALLBACK_PALETTE } = await Promise.resolve().then(() => __importStar(require('./constants/GlobalArtStyle')));
        try {
            const response = await this.openai.chat.completions.create({
                model: models_1.MODEL_CONFIG.TEXT,
                messages: [
...
Palette arc should mirror emotional arc.`
    ...
    async generateStoryImages(story, characterName, characterTraits, characterId, mode = 'batch', existingImages, storyId, onProgress) {
        try {
            // 1. FETCH CHARACTER REFERENCE IMAGES + INCLUSIVITY TRAITS (per plan Phase 9)
            let characterReferences = [];
            let characterInclusivityTraits = [];
            if (characterId) {
                try {
                    const { CharacterDatabase } = await Promise.resolve().then(() => __importStar(require('./database/CharacterDatabase')));
                    const charDb = new CharacterDatabase(this.config.supabase.url, this.config.supabase.anonKey, this.logger);
                    const savedChar = await charDb.getCharacterById(characterId);
                    if (savedChar) {
                        // Get validated reference images
                        if (savedChar.referenceImages) {
                            characterReferences = savedChar.referenceImages
                                .filter((ref) => ref.traitsValidated !== false)
                                .map((ref) => ref.url)
                                .filter(Boolean);
                        }
                        else if (savedChar.headshotUrl || savedChar.bodyshotUrl) {
                            // Fallback to legacy fields
                            if (savedChar.headshotUrl)
                                characterReferences.push(savedChar.headshotUrl);
                            if (savedChar.bodyshotUrl)
                                characterReferences.push(savedChar.bodyshotUrl);
                        }
                        // Get inclusivity traits for validation
                        if (savedChar.traits?.inclusivityTraits) {
                            const { INCLUSIVITY_TRAITS_MAP } = await Promise.resolve().then(() => __importStar(require('./constants/ComprehensiveInclusivityDatabase')));
                            characterInclusivityTraits = savedChar.traits.inclusivityTraits
                                .map((t) => INCLUSIVITY_TRAITS_MAP[t.type || t])
                                .filter(Boolean);
                        }
            ...
            const characterContext = await this.buildCharacterConsistencyContext(characterName, characterTraits);
            // Import GLOBAL_STYLE and STYLE_GUARDRAILS; keep GLOBAL_STYLE unchanged but apply guardrails once
            const { GLOBAL_STYLE, STYLE_GUARDRAILS, FALLBACK_PALETTE, CAMERA_ANGLES } = await Promise.resolve().then(() => __importStar(require('./constants/GlobalArtStyle')));
            ...
            const fullStoryText = story.keyBeats.map((b) => b.description || '').join('\n\n');
            ...
            const coverSceneAnalysis = await this.analyzeSceneForVisualDynamism(fullStoryText, 'cover', true, // isCover = true
            fullStoryText);
            ...
            const hasDisabilityTraits = Array.isArray(characterTraits?.inclusivityTraits) && characterTraits.inclusivityTraits.length > 0;
            const disabilityRepresentationBlock = hasDisabilityTraits
                ? `
...
- Adventure happens WITH the aid, not despite it
- Prosthetics stay attached during action - they are part of the body
- Hearing aids, glasses, medical devices remain visible and integrated`.trimEnd()
                : '';
            let coverPrompt = `PROTAGONIST DNA:
${characterContext}
...
PALETTE: ${paletteJourney[0]}

${this.buildMotifInstruction(motif)}${disabilityRepresentationBlock ? `\n\n${disabilityRepresentationBlock}` : ''}
...
${GLOBAL_STYLE}`;
            ...
            const existingCover = existingImages?.find((img) => img.beatNumber === 0);
            if (existingCover) {
                ...
                if (storyId && onProgress) {
                    ...
                    try {
                        ...
                        if (assetStatuses.every((a) => a.status === 'ready')) {
                            currentStatus.overall = 'ready';
                        }
                        else if (assetStatuses.some((a) => a.status === 'generating')) {
            ...
            if (coverUrl.includes('assets.storytailor.dev')) {
                ...
                const { getAssetBucketName } = await Promise.resolve().then(() => __importStar(require('./utils/cdnUrl')));
            ...
            const previousPoses = ['Cover: opening scene'];
            // Determine which beats to generate based on mode (per plan)
            const beatsToGenerate = mode === 'progressive'
                ? story.keyBeats.filter((beat) => !existingImages?.some((img) => img.beatNumber === beat.beatNumber))
            ...
            for (let i = 0; i < beatsToGenerate.length; i++) {
                ...
                const beatSceneAnalysis = await this.analyzeSceneForVisualDynamism(beat.description || beat.visualDescription || '', `beat_${beatIndex + 1}`, false // isCover = false
                );
                this.logger.info(`Beat ${beatIndex + 1} scene analysis complete`, {
                    beatNumber: beat.beatNumber,
                    analysisLength: beatSceneAnalysis.length
                });
                const beatPrompt = `PROTAGONIST DNA:
${characterContext}
...
PALETTE: ${paletteJourney[paletteIndex] || paletteJourney[1]}

${this.buildMotifInstruction(motif)}${disabilityRepresentationBlock ? `\n\n${disabilityRepresentationBlock}` : ''}

POSE VARIATION (MANDATORY):
- Distinct pose + camera variation across cover + beats
- Distinct composition/framing (wide, low angle, side view, top-down, or OTS)
- Clear limb + torso change (arms/hands/torso orientation)
- Avoid centered portrait framing; favor diagonal staging, foreshortening, or off-axis camera when appropriate.
...
${GLOBAL_STYLE}`;
     ...
     */
    async buildCharacterConsistencyContext(characterName, characterTraits) {
        ...
        if (characterTraits.inclusivityTraits && characterTraits.inclusivityTraits.length > 0) {
            try {
                ...
                characterTraits.inclusivityTraits.forEach((trait) => {
...
Visual consistency is paramount for therapeutic storytelling.`;
    ...
    async generateAudioNarration(text, attempt = 1) {
        const narrationModelId = (0, ElevenLabsModelConfig_1.getModelForUseCase)('story_narration');
        ...
        try {
            ...
            const narrationVoiceSettings = (0, ElevenLabsModelConfig_1.getVoiceSettingsForModel)(narrationModelId);
     ...
     */
    async generateAudioNarrationWithTimestamps(storyContent, attempt = 1) {
        const narrationModelId = (0, ElevenLabsModelConfig_1.getModelForUseCase)('story_narration');
        const MAX_RETRIES = narrationModelId === 'eleven_v3' ? 2 : 3;
        ...
        try {
            ...
            const controller = new AbortController();
            let timeoutId;
            const audioPromise = fetch(`https://api.elevenlabs.io/v1/text-to-speech/${this.elevenLabsConfig.voiceId}/with-timestamps?output_format=mp3_44100_128`, {
                method: 'POST',
                signal: controller.signal,
                headers: {
            ...
            for (let beatIndex = 0; beatIndex < storyContent.beats.length && beatIndex < beatKeys.length; beatIndex++) {
                ...
                const beatWords = beatText.split(/\s+/).filter(w => w.length > 0);
     ...
     */
    async persistImageToS3DirectWithCdn(imageBuffer, title) {
        try {
            // Upload to S3
            const { S3Client, PutObjectCommand } = await Promise.resolve().then(() => __importStar(require('@aws-sdk/client-s3')));
            const { getAssetBucketName, getCdnUrl } = await Promise.resolve().then(() => __importStar(require('./utils/cdnUrl')));
    ...
    computeStoryVersionHash(input) {
        return crypto
            .createHash('sha256')
            .update(JSON.stringify(input))
            .digest('hex');
    }
    computeAssetHashes(input) {
        const beatText = (idx) => String(input.keyBeats?.[idx]?.visualDescription || input.keyBeats?.[idx]?.description || '');
        ...
        const beatHashes = [0, 1, 2, 3].map((idx) => crypto
            .createHash('sha256')
            .update(JSON.stringify({
            title: input.title,
            characterName: input.characterName,
    ...
    async enqueueFinalizationJob(payload) {
    ...
    async processFinalizationJob(payload) {
        ...
        if (requiredCover && !story.coverImageUrl) {
            ...
            if (!coverImageUrl)
                throw new Error('Cover generation failed (empty URL)');
        ...
        if (requiredBeats > 0) {
            for (let beatNumber = 1; beatNumber <= requiredBeats; beatNumber++) {
                const hasBeat = Array.isArray(story.beatImages) &&
                    story.beatImages.some((b) => b?.beatNumber === beatNumber && b?.imageUrl);
                if (!hasBeat) {
                    const beatImageUrl = await this.generateBeatImageOnly(payload.storyId, beatNumber, story, characterTraits);
                    if (!beatImageUrl)
                        throw new Error(`Beat ${beatNumber} generation failed (empty URL)`);
        ...
        if (requiredAudio && !story.audioUrl) {
            ...
            if (audio.audioUrl) {
                ...
                const metadataPatch = { audioProvider: audio.provider };
    ...
    async generateKeyBeatsFromStoryText(storyText) {
        if (!storyText.trim()) {
...
Each beat must be a vivid, image-ready moment with a detailed visualDescription.`;
        ...
        const content = response.choices[0]?.message?.content;
        if (!content)
            throw new Error('No key beat content generated');
        const parsed = JSON.parse(content);
        ...
        const normalized = keyBeats.map((b, idx) => ({
    ...
    async generateLongFormAudioWithFallback(text) {
    ...
    chunkText(text, targetChars, hardMaxChars) {
    ...
    async generateChunkedAudioWithFallback(text, forcedProvider) {
        ...
        if (chunks.length === 0)
            return { audioUrl: '', provider: 'native_tts', error: 'No text to synthesize' };
        const segments = [];
    ...
    async synthesizePollyMp3(text) {
        const { PollyClient, SynthesizeSpeechCommand } = await Promise.resolve().then(() => __importStar(require('@aws-sdk/client-polly')));
    ...
    async updateStoryMetadata(storyId, patch) {
        const { data: currentStory, error: readError } = await this.supabase
            .from('stories')
            .select('content')
            .eq('id', storyId)
    ...
    async markAudioFailed(storyId, reason) {
        const { data: currentStory, error: readError } = await this.supabase
            .from('stories')
            .select('content')
            .eq('id', storyId)
    ...
    async updateStoryAudioUrl(storyId, audioUrl) {
        try {
            if (this.tempStoryCache.has(storyId)) {
                const tempStory = this.tempStoryCache.get(storyId);
    ...
    async updateAssetsStatusMetadata(storyId) {
        const { data: currentStory, error: readError } = await this.supabase
            .from('stories')
            .select('content')
            .eq('id', storyId)
        ...
        const assetsStatus = {
            ...
            beats: [
                existingAssetsStatus.beats?.[0] === 'failed' ? 'failed' : (beatImages.some((b) => b?.beatNumber === 1 && b?.imageUrl) ? 'ready' : 'pending'),
                existingAssetsStatus.beats?.[1] === 'failed' ? 'failed' : (beatImages.some((b) => b?.beatNumber === 2 && b?.imageUrl) ? 'ready' : 'pending'),
                existingAssetsStatus.beats?.[2] === 'failed' ? 'failed' : (beatImages.some((b) => b?.beatNumber === 3 && b?.imageUrl) ? 'ready' : 'pending'),
                existingAssetsStatus.beats?.[3] === 'failed' ? 'failed' : (beatImages.some((b) => b?.beatNumber === 4 && b?.imageUrl) ? 'ready' : 'pending'),
            ],
     ...
     */
    formatStoryResponseWithAttribution(baseMessage, ipAttributions) {
        if (ipAttributions.length === 0) {
            return baseMessage;
        }
        // Format attribution as quiet aside
        const attributionMessages = ipAttributions.map(attr => this.ipDetectionService.formatAttributionMessage(attr));
     ...
     */
    async checkAndSanitizeArtPrompt(artPrompt, userAge) {
        try {
            ...
            if (!this.contextualAnalyzer) {
                ...
                const { ContextualSafetyAnalyzer } = require('../../content-safety/src/filters/ContextualSafetyAnalyzer');
                this.contextualAnalyzer = new ContextualSafetyAnalyzer(this.openai, this.logger);
            }
            const safetyAnalysis = await this.contextualAnalyzer.analyzeContextualSafety(artPrompt, userAge, 'character');
