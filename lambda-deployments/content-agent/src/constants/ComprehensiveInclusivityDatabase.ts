/**
 * Comprehensive Inclusivity Database - Week 1 Core 20 Traits
 * Medical accuracy + AI bias mitigation + Dignity-first framing
 * 
 * This is Storytailor's competitive moat - authentic representation that combats
 * AI image generator bias toward "perfect" Euro-centric features.
 * 
...
export interface InclusivityTrait {
  ...
  // TIER 1: Visual + Medical (AI Image Generation)
  visualDescription: string;
  medicallyAccurateDescription: string;      // Detailed medical/physical specs
  gptImageSafePrompt: string;
  mandatoryVisualRequirements: string[];     // Features AI MUST include
  visualValidationChecklist: string[];       // What vision model verifies
  negativePrompt: string;                    // What AI must NOT do
  ...
  // TIER 2: Character + Story (Narrative Integration)
  personalityNuances: string[];              // How trait shapes personality
  strengthsFromTrait: string[];              // Empowerment framing
  conversationalHints: string[];
  storyIntegrationTips: string[];            // How to weave into plot
  
  // TIER 3: Cultural + Sensitivity (Respectful Portrayal)
  celebratoryLanguage: string;
  dignityFirstFraming: string;               // Person-first language
  avoidStereotypes: string[];                // What NOT to do
  culturalConsiderations?: string;           // Cultural sensitivity
  ageAppropriatenessNotes?: string;          // Age 3 vs 10 language
}
 ...
 */
export const CORE_INCLUSIVITY_TRAITS: InclusivityTrait[] = [
...
The goal is respectful representation celebrating neurodiversity and authentic autistic experience.`,
    ...
    mandatoryVisualRequirements: [
      ...
      'Eye contact optional (may look away - this is authentic)',
      'If character stims: Show naturally (not as problem)',
      'MUST show engaged in meaningful way (even if differently)'
    ],
...
- Portrayed as needing to be calm/still`,
    ...
    mandatoryVisualRequirements: [
      'If using fidget tool: MUST be visible',
      'Expression MUST show positive energy (not negative)',
      'Body language MUST show natural ADHD presentation',
      'MUST NOT portray as misbehaving'
    ],
...
- Shown as frustrated with reading only (show strengths too)`,
    ...
    mandatoryVisualRequirements: [
      'MUST show intelligence clearly',
      'If reading supports used: Show them naturally',
      'MUST show engaged in activities demonstrating intelligence'
    ],
...
DO NOT show without strength and courage`,
...

  {
    ...
    visualDescription: 'Insulin pump clipped to clothing',
    
    medicallyAccurateDescription: `Type 1 diabetes managed with insulin pump or injections. The character has Type 1 diabetes, naturally reflected by insulin pump clipped to clothing or medical alert bracelet. These devices enable them to live fully active lives.`,
    
    gptImageSafePrompt: `The character has Type 1 diabetes with insulin pump visible.

Insulin pump: Small device clipped to [waistband/pocket/belt], with thin tubing to infusion site. Medical alert bracelet may be visible. Character engaged confidently in activities, managing diabetes independently.

Diabetes management shown as natural part of daily life enabling full activity.`,
    
    mandatoryVisualRequirements: [
      'Insulin pump MUST be visible if worn',
      'Character MUST look confident and active',
      'MUST NOT portray as limiting'
    ],
...
DO NOT portray as fragile or limited`,
...

  {
    ...
    visualDescription: 'Inhaler in hand or holder',
    
    medicallyAccurateDescription: `Asthma managed with rescue inhaler. Character has asthma with inhaler visible or in holder/pocket.`,
    
    gptImageSafePrompt: `Character has asthma with inhaler visible. Inhaler in hand, pocket, or holder on belt. Character active and confident, inhaler enables full participation.`,
    
    mandatoryVisualRequirements: ['Inhaler visible if specified', 'Character active and capable'],
    visualValidationChecklist: ['Is inhaler visible?', 'Is character active?'],
...
Nasal cannula visible: Thin clear tubes in both nostrils, tubes loop over ears, connect to portable oxygen tank. MEDICAL breathing support, NOT decorative tubes.`,
...

  {
    ...
    visualDescription: 'Intravenous line in arm or chest for medication',
...
External processor visible: Rectangular device magnetically attached behind ear, microphone component, connecting to implant under skin. MEDICAL hearing device NOT consumer headphones.`,
...

  {
    ...
    visualDescription: 'Medical helmet for skull shaping',
 ...
 * Create map for easy lookup by ID
 */
export const INCLUSIVITY_TRAITS_MAP: Record<string, InclusivityTrait> = CORE_INCLUSIVITY_TRAITS.reduce(
  (map, trait) => {
    map[trait.id] = trait;
    return map;
  },
  {} as Record<string, InclusivityTrait>
);

/**
 * Get trait by ID with fallback
 */
export function getInclusivityTrait(id: string): InclusivityTrait | null {
  return INCLUSIVITY_TRAITS_MAP[id] || null;
}

/**
 * Get all traits by category
 */
export function getTraitsByCategory(category: InclusivityTrait['category']): InclusivityTrait[] {
  return CORE_INCLUSIVITY_TRAITS.filter(t => t.category === category);
}

/**
 * Search traits by label or description
 */
export function searchTraits(query: string): InclusivityTrait[] {
  const lowerQuery = query.toLowerCase();
  return CORE_INCLUSIVITY_TRAITS.filter(
    t => t.label.toLowerCase().includes(lowerQuery) ||
