...
import { VoiceDesignService, CharacterVoiceProfile } from './VoiceDesignService';
import { VoiceRemixingService } from './VoiceRemixingService';
...
export interface StoryCharacterVoice {
  ...
  remixVariations?: Map<string, string>; // context -> voiceId
}
...
export class CharacterVoiceGenerator {
  ...
  constructor(logger: Logger) {
    this.logger = logger;
    this.voiceDesignService = new VoiceDesignService(logger);
    this.voiceRemixingService = new VoiceRemixingService(logger);
  }
  ...
  async generateStoryCharacterVoices(
    ...
    try {
      ...
      const storyCharacters: StoryCharacterVoice[] = [];
      ...
      for (const characterProfile of characterProfiles) {
        ...
        const remixVariations = await this.voiceRemixingService.createCharacterRemixVariations(
          characterProfile.characterName,
          baseVoiceId,
          characterProfile.personality || 'friendly',
          storyType
        );

        const storyCharacter: StoryCharacterVoice = {
          characterName: characterProfile.characterName,
          voiceId: baseVoiceId,
      ...
      let narratorVoiceId: string | undefined;
      try {
        const narratorProfile: CharacterVoiceProfile = {
        ...
        if (narratorDesign.previews.length > 0) {
          narratorVoiceId = await this.voiceDesignService.createVoiceFromDesign(
            narratorDesign.previews[0].generated_voice_id,
            `Narrator_${storyType}`,
            narratorProfile.voiceDescription
          );
        }
      } catch (error) {
      ...
      const storyVoiceProfile: StoryVoiceProfile = {
        storyId: storyId,
        storyType: storyType,
  ...
  async refineCharacterVoice(
    ...
    try {
      ...
      const refinedVoiceId = await this.voiceRemixingService.iterativeRefinement(
        voiceId,
        refinementPrompts
      );
  ...
  async createQuickCharacterVoice(
    ...
    try {
      ...
      const characterProfile: CharacterVoiceProfile = {
      ...
      const voiceId = await this.voiceDesignService.createVoiceFromDesign(
        designResult.previews[0].generated_voice_id,
        `${characterName}_${personality}`,
        characterProfile.voiceDescription
      );
   ...
   */
  private getBackgroundMusicStyle(storyType: string): string {
    switch (storyType) {
      case 'adventure':
