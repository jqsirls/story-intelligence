import { Story, StoryContent, StoryBeat, Character } from '@alexa-multi-agent/shared-types';
// TODO: Fix voice-synthesis package build errors
// import { VoiceService } from '@alexa-multi-agent/voice-synthesis';
// import { VoiceSynthesisRequest, VoiceSynthesisResponse, AudioChunk } from '@alexa-multi-agent/voice-synthesis';

// Temporary interfaces until voice-synthesis builds
...
export class AudioGenerationService {
  ...
  async streamStoryNarration(
    ...
    try {
      ...
      if (this.config.voiceService.stream) {
        ...
        throw new Error('Voice service does not support streaming');
      }
  ...
  private prepareVoiceSettings(
    ...
    // Merge with custom settings and provide defaults
    return {
      voiceId: baseVoiceId,
      stability: 0.75,
  ...
  private getVoiceSettingsForMood(mood: string): Partial<VoiceCustomization> {
    const moodSettingsMap: Record<string, Partial<VoiceCustomization>> = {
      'excited': {
        stability: 0.5, // ElevenLabs TTD requires 0.0, 0.5, or 1.0
        similarityBoost: 0.8,
        style: 0.8,
        useSpeakerBoost: true,
        speed: 1.1,
      ...
      'calm': {
        stability: 1.0, // ElevenLabs TTD requires 0.0, 0.5, or 1.0
        similarityBoost: 0.7,
        style: 0.3,
        useSpeakerBoost: true,
        speed: 0.9,
  ...
  private async generateSegmentAudio(
    ...
    for (const segment of segments) {
      try {
        ...
        const url = response.audioUrl;
        if (process.env.NODE_ENV === 'production') {
          if (!url || /\/placeholders\//.test(url)) {
            throw new Error('TTS segment produced placeholder or empty audioUrl in production');
          }
        }
  ...
  private async generateFullNarration(
    ...
    const url = response.audioUrl || process.env.DEFAULT_AUDIO_URL || '';
    if (process.env.NODE_ENV === 'production') {
      if (!url || /\/placeholders\//.test(url)) {
        throw new Error('TTS produced placeholder or empty audioUrl in production');
      }
    }
