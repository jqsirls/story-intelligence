...
export interface SoundEffect {
  ...
  speaker?: string; // Specific speaker ID for spatial audio
}

export interface SpatialAudioSetup {
  mainDevice: string;              // Primary Echo (avatar/narration)
  surroundSpeakers?: string[];     // Additional Echos for spatial effects
}
...
export class SoundEffectsService {
   ...
   */
  async analyzeSoundEffectMoments(storyText: string): Promise<SoundEffect[]> {
...
Return JSON: {"effects": [{"effect": "birds", "timestamp": 5, "description": "Forest scene opens", "duration": 3, "volume": 0.3}]}`
      ...
      const effects: SoundEffect[] = await Promise.all(
   ...
   */
  async orchestrateMultiSpeaker(
    setup: SpatialAudioSetup,
   ...
   */
  async generateAmbianceTrack(storyType: string): Promise<string | null> {
   ...
   */
  private async uploadSoundEffectToS3(audioBuffer: Buffer, effectName: string): Promise<string> {
   ...
   */
  async createImmersiveAudio(
    storyText: string,
    storyType: string,
    spatialSetup?: SpatialAudioSetup
  ): Promise<{
    soundEffects: SoundEffect[];
