...
export class ColorExtractionService {
   ...
   */
  async extractDeepContrastingColors(imageUrl: string): Promise<{
    primary: string;
    secondary: string;
    tertiary: string;
  }> {
    try {
      ...
      if (imageUrl.includes('.s3.amazonaws.com/')) {
        ...
        if (!s3UrlParts) throw new Error(`Invalid S3 URL: ${imageUrl}`);
      ...
      const selectedColors: typeof deepColors = [];
      ...
      const hexColors = selectedColors.slice(0, 3).map(color => {
        const toHex = (n: number) => Math.min(255, Math.max(0, Math.floor(n))).toString(16).padStart(2, '0').toUpperCase();
   ...
   */
  async extractWithCharacterConsistency(
    imageUrl: string,
