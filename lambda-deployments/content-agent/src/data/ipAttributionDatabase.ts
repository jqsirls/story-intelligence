...
export interface IPAttributionEntry {
  franchise: string;
  owner: string;
  variants?: string[]; // Alternative names/spellings
}

export const IP_ATTRIBUTION_DB: Record<string, IPAttributionEntry> = {
  // Marvel Characters
 ...
 */
export function getIPAttribution(characterName: string): IPAttributionEntry | null {
  const normalizedName = characterName.trim();
  
  // Direct lookup
  if (IP_ATTRIBUTION_DB[normalizedName]) {
    return IP_ATTRIBUTION_DB[normalizedName];
  }
 ...
 */
export function getCharactersByFranchise(franchise: string): string[] {
  return Object.entries(IP_ATTRIBUTION_DB)
 ...
 * @returns IP attribution entry if found, null otherwise
 */
export function fuzzyMatchIP(characterName: string): IPAttributionEntry | null {
