...
const initializeAgent = async (): Promise<IdPAgent> => {
  if (!agent) {
    // Base URL for all internal APIs
    const baseUrl = process.env.API_BASE_URL || 'https://sxjwfwffz7.execute-api.us-east-1.amazonaws.com';
    ...
    const config = {
      ...
      tokenServiceUrl: process.env.TOKEN_SERVICE_URL || `${baseUrl}/token`,
      issuer: process.env.OIDC_ISSUER || `${baseUrl}/auth`,
      authorizationEndpoint: process.env.OIDC_AUTH_ENDPOINT || `${baseUrl}/auth/oauth/authorize`,
      tokenEndpoint: process.env.OIDC_TOKEN_ENDPOINT || `${baseUrl}/auth/oauth/token`,
      userinfoEndpoint: process.env.OIDC_USERINFO_ENDPOINT || `${baseUrl}/auth/oauth/userinfo`,
      jwksUri: process.env.OIDC_JWKS_URI || `${baseUrl}/auth/.well-known/jwks.json`,
      registrationEndpoint: process.env.OIDC_REGISTRATION_ENDPOINT || `${baseUrl}/auth/oauth/register`
    };
...
export const httpHandler = async (
  ...
  console.log('IdP HTTP handler invoked:', {
    ...
    requestId: (context as any).requestId || context.awsRequestId
  });
