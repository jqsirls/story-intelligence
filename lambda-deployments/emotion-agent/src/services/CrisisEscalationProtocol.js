...
exports.CrisisEscalationProtocol = void 0;
/**
 * Crisis escalation protocol for child safety and wellbeing
 * Requirements: 7.3, 7.4
 */
class CrisisEscalationProtocol {
     ...
     */
    async detectCrisisIndicators(userId, sessionId, analysisData) {
        try {
            ...
            if (analysisData.emotionalState) {
                const emotionalIndicators = await this.analyzeEmotionalStateForCrisis(analysisData.emotionalState, userId);
                indicators.push(...emotionalIndicators);
            }
            // Filter and prioritize indicators
            const significantIndicators = indicators.filter(indicator => indicator.confidence > 0.6 || indicator.severity === 'critical');
     ...
     */
    async executeCrisisResponse(indicators, context) {
        try {
     ...
     */
    async sendParentNotification(parentNotification, context) {
        try {
     ...
     */
    async analyzeVoiceForCrisis(voiceAnalysis) {
        const indicators = [];
        // Check for extreme distress in voice patterns
        if (voiceAnalysis.stressIndicators) {
            const highStressCount = voiceAnalysis.stressIndicators.filter((indicator) => indicator.severity === 'high').length;
        ...
        if (voiceAnalysis.emotionalMarkers) {
            const tremors = voiceAnalysis.emotionalMarkers.filter((marker) => marker.type === 'tremor' && marker.intensity > 0.7);
            if (tremors.length > 0) {
                indicators.push({
                    indicatorType: 'emotional_distress',
                    severity: 'medium',
     ...
     */
    async analyzeTextForCrisis(textContent) {
        const indicators = [];
     ...
     */
    async analyzeBehaviorForCrisis(behavioralPatterns) {
        const indicators = [];
        // Check for sudden behavioral changes
        if (behavioralPatterns.significantChanges) {
            const severeChanges = behavioralPatterns.significantChanges.filter((change) => change.magnitude > 2.0);
            if (severeChanges.length > 0) {
                indicators.push({
                    ...
                    evidence: severeChanges.map((change) => change.description),
                    context: {}
                });
            }
        }
     ...
     */
    async analyzeEmotionalStateForCrisis(emotionalState, userId) {
        const indicators = [];
     ...
     */
    assessOverallRiskLevel(indicators) {
     ...
     */
    async generateEscalationActions(indicators, context, riskLevel) {
        const actions = [];
     ...
     */
    generateImmediateResponse(riskLevel, indicators) {
        if (riskLevel === 'critical') {
     ...
     * Prepare parent notification
     */
    async prepareParentNotification(indicators, context, riskLevel) {
     ...
     */
    generateParentNotificationMessage(indicators, context, riskLevel) {
     ...
     */
    determineProfessionalReferral(indicators, riskLevel) {
        if (riskLevel === 'low') {
            return undefined;
        }
     ...
     * Create follow-up schedule
     */
    createFollowUpSchedule(riskLevel, indicators) {
        const now = new Date();
        const scheduleMap = {
            'critical': {
     ...
     */
    async logCrisisIndicators(userId, sessionId, indicators) {
        // Implementation for logging crisis indicators
    }
    requiresDocumentation(indicators, riskLevel) {
        return riskLevel === 'critical' || riskLevel === 'high' ||
            indicators.some(i => i.indicatorType === 'abuse_disclosure' || i.indicatorType === 'safety_risk');
    }
    async executeImmediateActions(crisisResponse) {
        // Implementation for executing immediate actions
    }
    async storeCrisisResponse(crisisResponse) {
        // Implementation for storing crisis response
    }
    async sendEmailNotification(email, notification, context) {
        // Implementation for sending email notification
        return true;
    }
    async sendPhoneNotification(phone, notification, context) {
    ...
    generateTriggerSigns(indicators, userAge) {
        // Implementation for generating trigger signs
        return [];
    }
    generateCopingStrategies(indicators, userAge) {
        // Implementation for generating coping strategies
        return [];
    }
    async identifySupportContacts(userId) {
        // Implementation for identifying support contacts
        return [];
    }
    async identifyProfessionalContacts(userId) {
        // Implementation for identifying professional contacts
        return [];
    }
    generateSafeEnvironmentSteps(userAge) {
    ...
    async storeSafetyPlan(safetyPlan) {
        // Implementation for storing safety plan
    }
}
exports.CrisisEscalationProtocol = CrisisEscalationProtocol;
//# sourceMappingURL=CrisisEscalationProtocol.js.map