...
export class VoicePatternAnalyzer {
  ...
  async analyzeVoicePatterns(audioData: AudioData): Promise<VoicePatternAnalysis> {
    try {
      ...
      const detectedEmotions = this.determineEmotionsFromPatterns(
   ...
   */
  private async extractVoiceCharacteristics(audioData: AudioData): Promise<VoiceCharacteristics> {
   ...
   */
  private async detectEmotionalMarkers(
    audioData: AudioData, 
    voiceCharacteristics: VoiceCharacteristics
  ): Promise<EmotionalMarker[]> {
    ...
    if (voiceCharacteristics.pitch.variance > 30 && voiceCharacteristics.volume.dynamicRange > 0.2) {
      const laughterConfidence = Math.min(0.9, 
        (voiceCharacteristics.pitch.variance / 50) * 0.5 + 
  ...
  private determineEmotionsFromPatterns(
    ...
    const emotions: Mood[] = [];
    const emotionScores: Record<Mood, number> = {
      happy: 0,
      sad: 0,
      scared: 0,
      angry: 0,
    ...
    if (emotions.length === 0) {
      const dominantEmotion = Object.entries(emotionScores).reduce((a, b) => 
        emotionScores[a[0] as Mood] > emotionScores[b[0] as Mood] ? a : b
      )[0] as Mood;
  ...
  private calculateOverallConfidence(
    ...
    // Marker confidence
    const markerConfidence = emotionalMarkers.reduce((sum, marker) => 
      sum + marker.confidence, 0) / Math.max(emotionalMarkers.length, 1);
    confidence += markerConfidence * 0.3;

    // Stress indicator confidence
    const stressConfidence = stressIndicators.reduce((sum, indicator) => 
      sum + indicator.confidence, 0) / Math.max(stressIndicators.length, 1);
