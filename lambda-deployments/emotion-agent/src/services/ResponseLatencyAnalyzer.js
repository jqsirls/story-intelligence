...
class ResponseLatencyAnalyzer {
    ...
    async analyzeEngagementMetrics(userId, sessionId) {
        try {
            ...
            const recommendations = this.generateEngagementRecommendations(engagementLevel, attentionSpan, fatigueIndicators);
            return {
                averageResponseTime,
    ...
    async detectInterventionTriggers(userId, sessionId) {
        try {
            ...
            const extremeLatencies = latencyData.filter(d => d.response_time > 15000 || d.response_time < 500);
     ...
     */
    async getSessionLatencyData(userId, sessionId) {
     ...
     */
    async analyzeRealTimeEngagement(userId, sessionId) {
        if (!this.redis) {
            return;
        }
        try {
            ...
            const recentLatencies = latencyHistory.slice(-3).map((d) => d.responseTime);
     ...
     */
    determineEngagementLevel(averageResponseTime, variance) {
     ...
     */
    estimateAttentionSpan(latencyData) {
        if (latencyData.length === 0) {
     ...
     */
    detectFatigueIndicators(latencyData) {
        const indicators = [];
     ...
     */
    generateEngagementRecommendations(engagementLevel, attentionSpan, fatigueIndicators) {
        const recommendations = [];
     ...
     */
    async analyzeSessionPattern(userId, sessionId, sessionData) {
        const latencyData = await this.getSessionLatencyData(userId, sessionId);
     ...
     */
    calculateOptimalInteractionTime(latencyData) {
        if (latencyData.length === 0) {
     ...
     */
    detectFatigueOnset(latencyData) {
        if (latencyData.length < 5) {
            return undefined;
        }
        const sessionStart = new Date(latencyData[0].created_at).getTime();
...
exports.ResponseLatencyAnalyzer = ResponseLatencyAnalyzer;
//# sourceMappingURL=ResponseLatencyAnalyzer.js.map