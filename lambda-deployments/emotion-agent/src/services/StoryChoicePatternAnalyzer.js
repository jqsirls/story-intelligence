...
class StoryChoicePatternAnalyzer {
    ...
    async analyzeEmotionalChoiceCorrelations(userId) {
        try {
            ...
            for (const [mood, moodChoices] of Object.entries(choicesByMood)) {
                if (moodChoices.length < 3)
                    continue; // Need at least 3 choices per mood
                const correlation = await this.calculateEmotionalCorrelation(mood, moodChoices);
     ...
     */
    async trackLongitudinalTrends(userId, timeRange) {
        try {
     ...
     */
    async detectChoiceBasedInterventionTriggers(userId) {
        try {
            ...
            let triggerType = 'behavioral_pattern';
            let severity = 'low';
            let description = '';
     ...
     */
    async getUserChoices(userId, timeRange) {
     ...
     */
    async updateChoicePatternCache(choice) {
        if (!this.redis)
            return;
        try {
     ...
     */
    async triggerPatternAnalysisIfReady(userId) {
        try {
            const recentChoices = await this.getUserChoices(userId, {
     ...
     */
    async analyzeRiskTakingPatterns(userId, choices) {
        const riskKeywords = ['adventure', 'explore', 'dangerous', 'unknown', 'brave', 'challenge', 'dare'];
        const riskChoices = choices.filter(choice => riskKeywords.some(keyword => choice.selected_choice.toLowerCase().includes(keyword)));
        ...
        const examples = riskChoices.slice(0, 3).map(choice => `"${choice.selected_choice}" at ${choice.choice_point}`);
     ...
     */
    async analyzeSafetySeekingPatterns(userId, choices) {
        const safetyKeywords = ['safe', 'careful', 'home', 'family', 'help', 'avoid', 'hide', 'protect'];
        const safetyChoices = choices.filter(choice => safetyKeywords.some(keyword => choice.selected_choice.toLowerCase().includes(keyword)));
        if (safetyChoices.length < 3) {
            return [];
        }
        const frequency = safetyChoices.length / choices.length;
        const confidence = Math.min(0.9, frequency * 2);
        const examples = safetyChoices.slice(0, 3).map(choice => `"${choice.selected_choice}" at ${choice.choice_point}`);
        const developmentalInsights = [];
        if (frequency > 0.7) {
     ...
     */
    async analyzeCreativeExplorationPatterns(userId, choices) {
        const creativeKeywords = ['create', 'imagine', 'magic', 'art', 'music', 'invent', 'dream', 'wonder'];
        const creativeChoices = choices.filter(choice => creativeKeywords.some(keyword => choice.selected_choice.toLowerCase().includes(keyword)));
        ...
        const examples = creativeChoices.slice(0, 3).map(choice => `"${choice.selected_choice}" at ${choice.choice_point}`);
        const developmentalInsights = [];
        if (frequency > 0.4) {
     ...
     */
    async analyzeSocialPreferencePatterns(userId, choices) {
        const socialKeywords = ['friend', 'together', 'share', 'help others', 'team', 'group', 'community'];
        const socialChoices = choices.filter(choice => socialKeywords.some(keyword => choice.selected_choice.toLowerCase().includes(keyword)));
        if (socialChoices.length < 2) {
            return [];
        }
        const frequency = socialChoices.length / choices.length;
        const confidence = Math.min(0.8, frequency * 2.5);
        const examples = socialChoices.slice(0, 3).map(choice => `"${choice.selected_choice}" at ${choice.choice_point}`);
        const developmentalInsights = [];
        if (frequency > 0.5) {
     ...
     */
    async analyzeProblemSolvingPatterns(userId, choices) {
        const problemSolvingKeywords = ['think', 'solve', 'figure out', 'plan', 'strategy', 'analyze', 'understand'];
        const problemSolvingChoices = choices.filter(choice => problemSolvingKeywords.some(keyword => choice.selected_choice.toLowerCase().includes(keyword)));
        if (problemSolvingChoices.length < 2) {
            return [];
        }
        const frequency = problemSolvingChoices.length / choices.length;
        const confidence = Math.min(0.8, frequency * 2.5);
        const examples = problemSolvingChoices.slice(0, 3).map(choice => `"${choice.selected_choice}" at ${choice.choice_point}`);
        const developmentalInsights = [];
        if (frequency > 0.4) {
     ...
     */
    async calculateEmotionalCorrelation(mood, choices) {
     ...
     */
    calculateChoiceConsistency(choices) {
        if (choices.length < 3)
            return 0.5;
     ...
     */
    analyzeConfidenceTrend(choices) {
        if (choices.length < 10)
            return null;
     ...
     */
    analyzePreferenceChanges(choices, timeRange) {
        ...
        const preferenceOverlap = earlyPatterns.preferred.filter(p => latePatterns.preferred.includes(p)).length;
     ...
     */
    analyzeEmotionalDevelopment(choices, timeRange) {
        const choicesWithEmotion = choices.filter(c => c.emotional_context);
        if (choicesWithEmotion.length < 8)
            return null;
     ...
     */
    calculateEmotionalComplexity(choices) {
     ...
     */
    identifyConcerningPatterns(choices) {
        const concerns = [];
        if (choices.length < 5)
            return concerns;
        // Check for consistently negative emotional contexts
        const negativeEmotions = choices.filter(c => ['sad', 'scared', 'angry'].includes(c.emotional_context));
        if (negativeEmotions.length > choices.length * 0.7) {
            concerns.push({
                type: 'emotional_distress',
                severity: negativeEmotions.length > choices.length * 0.8 ? 'high' : 'medium',
...
exports.StoryChoicePatternAnalyzer = StoryChoicePatternAnalyzer;
//# sourceMappingURL=StoryChoicePatternAnalyzer.js.map