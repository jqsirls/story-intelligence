...
type EmotionalGoal = MoodBasedRecommendationContext['emotionalGoal'];
type StoryMapEntry = {
  storyType: string;
  theme: string;
  tone: 'uplifting' | 'calming' | 'energetic' | 'gentle' | 'neutral';
...
export class MoodBasedStoryRecommendationEngine {
  ...
  private async generatePrimaryRecommendation(context: MoodBasedRecommendationContext): Promise<StoryRecommendation> {
    ...
    const moodStoryMappings: Record<Mood, Partial<Record<EmotionalGoal, StoryMapEntry>>> = {
      'sad': {
        'mood_improvement': {
    ...
    const defaultMapping: StoryMapEntry = {
      storyType: 'Educational',
      theme: 'discovery_wonder',
      tone: 'energetic',
      expectedImpact: 'Sparking curiosity and positive engagement'
    };
    const mapping: StoryMapEntry =
      moodMappingsForMood[emotionalGoal] ||
      moodMappingsForMood['mood_improvement'] ||
      (moodStoryMappings['neutral']['mood_improvement'] as StoryMapEntry) ||
      defaultMapping;
