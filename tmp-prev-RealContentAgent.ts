...
and notes atmosphere.
- MUST name the protagonist’s full-body pose using at least 2 limb-specific details (e.g., “left arm reaching”, “knees tucked”, “torso twisted”).
- MUST name a concrete camera setup: shot type + angle + lens/DoF (e.g., “low-angle wide 24mm”, “over-shoulder shallow focus”).
- MUST include strong staging: diagonal composition, foreshortening, or off-axis framing (avoid centered portrait framing).
Do NOT mention medium, palette, UI, or watermarks.
...
- MUST describe the protagonist’s full-body pose with at least 2 limb-specific details (e.g., “left arm reaching”, “knees tucked”, “torso twisted”).
- MUST specify composition like an illustrator: framing + viewpoint + depth-of-field (e.g., “wide low viewpoint”, “over-shoulder shallow focus”, “top-down with deep focus”).
- MUST avoid centered portrait framing; use diagonal staging, foreshortening, or off-axis framing when it fits the action.
...
export class RealContentAgent {
  ...
  constructor(private config: RealContentAgentConfig) {
    ...
    this.multiCharacterDialogueService = new MultiCharacterDialogueService(this.logger, elevenLabsKey);
    this.characterVoiceManager = new CharacterVoiceManager(this.logger);
    this.soundEffectsService = new SoundEffectsService(this.logger, elevenLabsKey);
    this.musicCompositionService = new MusicCompositionService(this.logger, elevenLabsKey);
    this.voiceDesignService = new VoiceDesignService(this.logger, elevenLabsKey);
    this.voiceRemixingService = new VoiceRemixingService(this.logger, elevenLabsKey);
    this.characterVoiceGenerator = new CharacterVoiceGenerator(this.logger, elevenLabsKey);
...
Palette arc should mirror emotional arc.`
  ...
  async generateStoryImages(
    ...
    try {
        ...
        try {
          ...
          if (savedChar) {
            ...
            if (savedChar.referenceImages) {
              ...
              if (savedChar.headshotUrl) characterReferences.push(savedChar.headshotUrl);
              if (savedChar.bodyshotUrl) characterReferences.push(savedChar.bodyshotUrl);
            }
      ...
      const { GLOBAL_STYLE, STYLE_GUARDRAILS, FALLBACK_PALETTE, CAMERA_ANGLES } = await import('./constants/GlobalArtStyle');
      ...
      const hasDisabilityTraits =
        Array.isArray(characterTraits?.inclusivityTraits) && characterTraits.inclusivityTraits.length > 0
...
Visual consistency is paramount for therapeutic storytelling.`;
  ...
  private async uploadAudioToS3(audioBuffer: Buffer): Promise<string> {
    try {
      ...
      const s3Client = new S3Client({ region: this.config.s3?.region || 'us-east-1' });
      // IMPORTANT: Audio must be served from assets.storytailor.dev (CloudFront), not a public S3 bucket URL.
      // Store audio in the assets bucket and return CDN URL.
      const bucketName = getAssetBucketName();
      const key = `stories/audio/${Date.now()}-${Math.random().toString(36).substring(7)}.mp3`;
