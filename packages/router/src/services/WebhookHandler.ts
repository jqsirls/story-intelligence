...
export class WebhookHandler {
  ...
  private platformRouter: any = null; // Lazy-loaded to avoid circular dependency
  ...
  constructor(private config: RouterConfig) {
    ...
    // Lazy-load PlatformAwareRouter to avoid circular dependency
    this.initializePlatformRouter();
  }

  private initializePlatformRouter(): void {
    try {
      // Only initialize if not already initialized to prevent recursion
      if (this.platformRouter) {
        return;
      }
      const PlatformAwareRouterModule = require('../PlatformAwareRouter');
      const PlatformAwareRouter = PlatformAwareRouterModule.PlatformAwareRouter || PlatformAwareRouterModule.default;
      if (PlatformAwareRouter) {
        // Check if we're being called from within PlatformAwareRouter constructor to avoid recursion
        const stack = new Error().stack || '';
        if (stack.includes('PlatformAwareRouter') && stack.includes('constructor')) {
          this.logger.debug('Skipping PlatformAwareRouter initialization to prevent recursion');
          return;
        }
        this.platformRouter = new PlatformAwareRouter(this.config);
        // Get platform adapters from the router
        this.platformAdapters = this.platformRouter['platformAdapters'] || new Map();
      }
    } catch (error: any) {
      // Check if error is stack overflow - if so, don't log as it will cause more recursion
      if (error.message && error.message.includes('Maximum call stack size exceeded')) {
        this.logger.debug('PlatformAwareRouter initialization skipped due to recursion prevention');
        return;
      }
      this.logger.warn('PlatformAwareRouter not available, continuing without platform-specific features', {
        error: error.message
      });
    }
  }
  ...
  private async handleSmartHomeControl(
    ...
    // Execute smart home control through the router (if available)
    let controlResult = null;
    if (this.platformRouter && this.platformRouter['smartHomeIntegrator']) {
      controlResult = await this.platformRouter['smartHomeIntegrator'].executeAction(
        payload.userId,
        {
          type: 'device_control',
          userId: payload.userId,
      ...
      this.logger.warn('SmartHomeIntegrator not available for webhook control', { userId: payload.userId });
    }
    ...
    try {
      ...
      const { error } = await this.supabase
        ...
        .insert({
          ...
          config: registration.config as any,
