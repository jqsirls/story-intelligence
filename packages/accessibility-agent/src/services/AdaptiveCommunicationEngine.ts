...
export class AdaptiveCommunicationEngine {
  ...
  async adaptResponse(
    ...
    try {
      ...
      throw new AdaptationError(`Failed to adapt response: ${error.message}`, { originalResponse, profile: profile.id });
    }
  }
  ...
  async processSpeechDelay(
    ...
    try {
      ...
      throw new AdaptationError(`Failed to process speech delay: ${error.message}`, { input, profile: profile.id });
    }
  }
