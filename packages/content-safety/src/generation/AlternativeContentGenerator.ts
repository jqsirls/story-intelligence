...
export class AlternativeContentGenerator {
  ...
  async generateAlternative(
    request: AlternativeContentRequest
    ...
    try {
      ...
      const analysis = this.analyzeAlternativeContent(
  ...
  private createAlternativeContentPrompt(request: AlternativeContentRequest): string {
...
Please provide only the rewritten content without any explanations or additional text.`;
  ...
  private generateGenericSafeContent(contentType: string, age?: number): string {
  ...
  async batchGenerateAlternatives(
    requests: AlternativeContentRequest[]
  ): Promise<AlternativeContentResult[]> {
    this.logger.info('Generating batch alternative content', { count: requests.length });

    const results = await Promise.all(
      requests.map(request => this.generateAlternative(request))
    );
  ...
  async healthCheck(): Promise<boolean> {
    ...
    try {
      ...
      const testRequest: AlternativeContentRequest = {
