...
export class HumanEscalationManager {
  ...
  private escalationTriggers: HumanEscalationTrigger[] = [];
  ...
  private initializeEscalationTriggers(): void {
    this.escalationTriggers = [
      ...
      },
      {
        ...
        condition: (result) => result.biasDetection?.overallBiasScore > 0.8,
        priority: 'high',
      ...
      },
      {
        ...
        condition: (result) => 
          result.flaggedCategories.includes('child_safety') ||
      ...
      },
      {
        ...
        condition: (result) => 
          result.qualityAssessment?.overallQuality < 0.4 &&
  ...
  async escalateForReview(
    ...
    try {
      ...
      const triggeredEscalations = this.escalationTriggers.filter(trigger =>
  ...
  private async sendEscalation(payload: any): Promise<void> {
    if (!this.webhookUrl) {
  ...
  private getHighestPriority(
    escalations: HumanEscalationTrigger[]
  ...
  private estimateReviewTime(priority: 'low' | 'medium' | 'high' | 'urgent'): string {
    const timeEstimates = {
      urgent: '15 minutes',
      high: '1 hour',
      medium: '4 hours',
  ...
  removeEscalationTrigger(triggerName: string): void {
    ...
    this.escalationTriggers = this.escalationTriggers.filter(
