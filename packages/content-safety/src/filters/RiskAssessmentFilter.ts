...
export class RiskAssessmentFilter implements PreGenerationFilter {
  ...
  async filter(request: ContentSafetyRequest): Promise<PreFilterResult> {
    ...
    try {
      ...
      if (request.contentType === 'story' && request.context?.previousContent) {
        const continuityRisk = await this.assessContinuityRisk(
          request.content,
          request.context.previousContent
        );
  ...
  private async assessRiskFactors(request: ContentSafetyRequest): Promise<{
    emotionalIntensity: number;
    complexityLevel: number;
    potentialTrauma: number;
    ...
    const complexityLevel = Math.min(1, 
      (this.calculateWordPresence(content, complexWords) + 
  ...
  private calculateWordPresence(content: string, words: string[]): number {
