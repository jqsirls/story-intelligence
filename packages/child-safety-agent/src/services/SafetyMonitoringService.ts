...
export class SafetyMonitoringService {
  ...
  async logSafetyIncident(incident: SafetyIncident): Promise<void> {
    try {
      ...
      await this.redis.lpush(cacheKey, JSON.stringify(incident));
      await this.redis.ltrim(cacheKey, 0, 99); // Keep last 100 incidents
  ...
  async getSafetyIncidents(userId: string, limit: number = 50): Promise<SafetyIncident[]> {
    try {
      ...
      const cachedIncidents = await this.redis.lrange(cacheKey, 0, limit - 1);
      
      if (cachedIncidents.length > 0) {
        return cachedIncidents.map(incident => JSON.parse(incident));
      }
