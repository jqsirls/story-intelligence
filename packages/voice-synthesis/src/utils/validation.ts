import { VoiceEngineConfig, VoiceError, VoiceErrorCode } from '../types';

/**
 * Validate voice engine configuration
 */
export function validateConfig(config: VoiceEngineConfig): void {
  const errors: string[] = [];
 ...
 */
export function validateSynthesisText(text: string): void {
...
export function validateVoiceId(voiceId: string): void {
  ...
  if (!uuidPattern.test(voiceId) && !pollyVoices.includes(voiceId)) {
    throw new VoiceError(
      ...
      `Invalid voice ID format: ${voiceId}`,
      undefined
    );
  }
}
...
export function validateSampleRate(sampleRate: number): void {
  ...
  if (!supportedRates.includes(sampleRate)) {
    throw new VoiceError(
      VoiceErrorCode.INVALID_REQUEST,
      `Unsupported sample rate: ${sampleRate}. Supported rates: ${supportedRates.join(', ')}`,
      undefined
    );
  }
}
 ...
 */
export function validateEmotion(emotion: string): void {
  const supportedEmotions = ['neutral', 'happy', 'sad', 'excited', 'calm', 'dramatic'];
  
  if (!supportedEmotions.includes(emotion)) {
    throw new VoiceError(
      VoiceErrorCode.INVALID_REQUEST,
      `Unsupported emotion: ${emotion}. Supported emotions: ${supportedEmotions.join(', ')}`,
      undefined
    );
  }
}
 ...
 */
export function sanitizeTextForLogging(text: string): string {
 ...
 */
export function validateBase64Audio(audioData: string): void {
  if (!audioData) {
    throw new VoiceError(
      VoiceErrorCode.INVALID_REQUEST,
  ...
  if (sizeBytes > 10 * 1024 * 1024) {
    throw new VoiceError(
      VoiceErrorCode.INVALID_REQUEST,
      'Audio sample too large (max 10MB)',
      undefined
    );
  }
}