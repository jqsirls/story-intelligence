...
class CostTracker {
    constructor(config, logger) {
        ...
        this.userCosts = new Map(); // userId -> daily cost
     ...
     */
    async checkLimits(userId, characterCount) {
        ...
        if (estimatedCost > this.config.maxCostPerRequest) {
            throw new types_1.VoiceError(types_1.VoiceErrorCode.QUOTA_EXCEEDED, `Request cost ($${estimatedCost.toFixed(4)}) exceeds per-request limit ($${this.config.maxCostPerRequest})`, 'elevenlabs');
        }
        ...
        if (currentDailyCost + estimatedCost > this.config.dailyBudgetLimit) {
            throw new types_1.VoiceError(types_1.VoiceErrorCode.QUOTA_EXCEEDED, `Request would exceed daily budget limit ($${this.config.dailyBudgetLimit}). Current: $${currentDailyCost.toFixed(2)}`, 'elevenlabs');
        }
        ...
        if (userId) {
            ...
            if (userDailyCost + estimatedCost > userDailyLimit) {
                throw new types_1.VoiceError(types_1.VoiceErrorCode.QUOTA_EXCEEDED, `User daily limit exceeded ($${userDailyLimit.toFixed(2)}). Current: $${userDailyCost.toFixed(2)}`, 'elevenlabs');
            }
        }
    }
    /**
     ...
     */
    estimateRequestCost(characterCount) {
