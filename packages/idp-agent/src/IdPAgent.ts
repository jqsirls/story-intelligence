...
export class IdPAgent {
  ...
  private async validateAuthorizationRequest(
    ...
    if (!client.allowed_response_types.includes(request.response_type)) {
      return {
    ...
    if (request.code_challenge_method && 
        !client.allowed_code_challenge_methods.includes(request.code_challenge_method)) {
      return {
        error: 'invalid_request',
  ...
  private async checkUserConsent(
    ...
    return requestedScopes.every(s => consent.granted_scopes.includes(s));
  }
  ...
  private async handleAuthorizationCodeGrant(
    ...
    const tokens = await this.generateTokens({
      ...
      includeRefreshToken: authCode.scope.includes('offline_access')
    });
  ...
  private async handleClientCredentialsGrant(
    ...
    const tokens = await this.generateTokens({
      ...
      clientId: client.client_id,
      scope: request.scope || client.allowed_scopes.join(' '),
  ...
  private async generateTokens(params: {
    ...
    const tokenServiceEvent = await this.eventBridge.putEvents({
      Entries: [{
  ...
  private async fetchUserInfo(userId: string, scope: string): Promise<UserInfo> {
    ...
    if (scopes.includes('phone')) {
      userInfo.phone_number = claims?.phone_number;
      userInfo.phone_number_verified = claims?.phone_number_verified;
    }

    if (scopes.includes('address') && claims?.address_formatted) {
      userInfo.address = {
        formatted: claims.address_formatted,
        street_address: claims.address_street,
        locality: claims.address_locality,
        region: claims.address_region,
        postal_code: claims.address_postal_code,
    ...
    if (scopes.includes('storytailor:characters')) {
      userInfo.character_ids = claims?.character_ids;
    }

    if (scopes.includes('storytailor:family')) {
      userInfo.family_id = claims?.family_id;
    }
