...
export class ContentAgent {
  ...
  private createAssetPipelineConfig(): AssetGenerationPipelineConfig {
    return {
      ...
      audioGeneration: {
        voiceService: {} as any, // Would be injected from voice-synthesis package
  ...
  async generateSpecificAssets(
    ...
    return await this.generateStoryAssets(request);
  }etStoryThemes(storyType);
    const matches = therapeuticNeeds.filter(need => 
      storyThemes.some(theme => theme.toLowerCase().includes(need.toLowerCase()))
    );
    
    return matches.length / therapeuticNeeds.length;
  }
  ...
  private getStorySafetyLevel(storyType: StoryType): 'safe' | 'moderate' | 'sensitive' {
    const sensitiveTypes = ['Child Loss', 'Inner Child', 'Medical Bravery', 'Mental Health'];
    if (sensitiveTypes.includes(storyType)) {
      return 'sensitive';
    }
    
    const moderateTypes = ['New Birth', 'Milestones'];
    if (moderateTypes.includes(storyType)) {
      return 'moderate';
    }
