...
exports.InactivityMonitorService = void 0;
const EmailService_1 = require("./EmailService");
class InactivityMonitorService {
    constructor(supabase, logger, emailService) {
        ...
        this.emailService = emailService || new EmailService_1.EmailService(supabase, logger);
    }
    /**
        ...
        try {
            ...
            for (const tier of userTiers || []) {
                ...
                try {
                    ...
                    const threshold = this.getThresholdForTier(tier.tier);
                    if (daysInactive >= threshold) {
                        // Send warning email
                        await this.sendInactivityWarning(tier.user_id, tier.tier, daysInactive);
                        warningsSent++;
                    }
                }
                catch (error) {
     ...
     */
    async sendInactivityWarning(userId, tier, monthsInactive) {
        try {
            // Get user info
     ...
     */
    async trackEngagement(userId, engagementType, engagementToken, clickUrl) {
        try {
     ...
     */
    async resetInactivityTimer(userId) {
        try {
            await this.supabase
                .from('user_tiers')
                .update({
     ...
     */
    generateEngagementToken(userId, emailType) {
        const crypto = require('crypto');
        const token = crypto.randomBytes(32).toString('hex');
        return `${userId}-${emailType}-${token}`;
    }
}
exports.InactivityMonitorService = InactivityMonitorService;
//# sourceMappingURL=InactivityMonitorService.js.map